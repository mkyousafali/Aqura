...
class PushNotificationProcessor {
  ...
  start() {
    console.log(
      `ℹ️ Push notification processor v${PushNotificationProcessor.VERSION} - Client-side processing ENABLED`,
    );
    console.log(
      `📱 Notifications will appear when app is open (browser notifications)`,
    );
    console.log(
      `⚠️ Note: Notifications require app to be open. Edge Function approach needs more work.`,
    );
    ...
    console.log(
      `🚀 Starting push notification processor v${PushNotificationProcessor.VERSION}`,
    );
  ...
  private async processFallbackSubscription(queueItem: any): Promise<boolean> {
    try {
      console.log(
        `🔄 Looking for fallback subscription for user ${queueItem.user_id}`,
      );
      console.log(
        `   Current queue item - Device: ${queueItem.device_id}, Subscription: ${queueItem.push_subscription_id}`,
      );
      ...
      const { data: latestSubscription, error } = await supabase
        .from("push_subscriptions")
        .select(
          "id, device_id, endpoint, p256dh, auth, device_type, last_seen, created_at",
        )
      ...
      if (error || !latestSubscription) {
        console.warn(
          `⚠️ No active subscription found for user ${queueItem.user_id}`,
        );
        return false;
      }
      ...
      console.log(
        `   Device: ${latestSubscription.device_type} (${latestSubscription.device_id})`,
      );
      ...
      const isDeviceIdDifferent =
        queueItem.device_id !== latestSubscription.device_id;
      const isSubscriptionIdDifferent =
      ...
      if (isDeviceIdDifferent || isSubscriptionIdDifferent) {
        ...
        if (isDeviceIdDifferent) {
          console.log(
            `   Device ID: ${queueItem.device_id} → ${latestSubscription.device_id}`,
          );
        }
        if (isSubscriptionIdDifferent) {
          console.log(
            `   Subscription ID: ${queueItem.push_subscription_id} → ${latestSubscription.id}`,
          );
        }
        ...
        if (updateError) {
          console.error(
            `❌ Failed to update queue item with fallback subscription:`,
            updateError,
          );
          return false;
        }
        ...
        console.log(
          `✅ Queue item ${queueItem.id} updated with fallback subscription`,
        );
      } else {
        console.log(
          `ℹ️ Queue item already has the latest subscription details`,
        );
    ...
    try {
      ...
      for (const notification of recentNotifications) {
        ...
        if (!existingQueue || existingQueue.length === 0) {
          console.log(
            `🔄 Creating fallback queue for notification ${notification.id}: ${notification.title}`,
          );
          ...
          if (createError) {
            console.error(
              `❌ Failed to create fallback queue for notification ${notification.id}:`,
              createError,
            );
          } else {
            console.log(
              `✅ Fallback queue created for notification ${notification.id}`,
            );
          }
        }
      }
  ...
  private async processNotificationWithRetry(queueItem: any) {
    try {
      ...
      console.log(
        `🎯 Processing notification ${queueItem.id} (attempt ${currentRetryCount + 1}/${maxRetries})`,
      );
      ...
      if (!queueItem.push_subscription_id) {
        console.warn(
          "⚠️ Queue item has no push_subscription_id:",
          queueItem.id,
        );
        await this.markNotificationFailed(
          queueItem.id,
          "No push subscription ID",
        );
        return;
      }
      ...
      if (!subscription || subError) {
        console.warn(
          "⚠️ No active push subscription found for queue item:",
          queueItem.id,
          "subscription_id:",
          queueItem.push_subscription_id,
        );
        await this.markNotificationFailed(
          queueItem.id,
          "Push subscription not found or inactive",
        );
        return;
      }
      ...
      try {
        ...
        console.log(
          `✅ Notification ${queueItem.id} sent successfully on attempt ${currentRetryCount + 1}`,
        );
        ...
        console.error(
          `❌ Attempt ${currentRetryCount + 1} failed for notification ${queueItem.id}:`,
          error,
        );
        ...
        if (currentRetryCount < maxRetries - 1) {
          ...
          nextRetryAt.setSeconds(
            nextRetryAt.getSeconds() + retryIntervalSeconds,
          );
          ...
          console.log(
            `🔄 Notification ${queueItem.id} scheduled for retry ${currentRetryCount + 2}/${maxRetries} in ${retryIntervalSeconds} seconds`,
          );
        } else {
          ...
          console.log(
            `❌ Notification ${queueItem.id} permanently failed after ${maxRetries} attempts`,
          );
        }
      }
    } catch (error) {
  ...
  private async sendPushNotification(queueItem: QueuedNotification) {
    ...
    try {
      console.log(
        `📤 Sending push notification ${queueItem.id} to device ${queueItem.device_id}...`,
      );
      ...
      if ("serviceWorker" in navigator && "Notification" in window) {
        try {
          ...
          console.log(
            `🔍 Using ${timeoutMs / 1000}s timeout for ${isProduction ? "production" : "development"} environment`,
          );
          ...
          try {
            ...
            console.log(
              `🔍 Found ${existingRegistrations.length} existing registrations`,
            );

            // Look for any registration first (active, waiting, or installing) - more lenient in production
            const availableRegistration = existingRegistrations.find(
              (reg) =>
              ...
              const timeoutPromise = new Promise((_, reject) =>
                setTimeout(
                  () =>
                    reject(
                      new Error(
                        `Service Worker registration timeout after ${timeoutMs / 1000}s`,
                      ),
                    ),
                  timeoutMs,
                ),
            ...
            console.warn(
              "⚠️ Service Worker not ready, attempting manual registration...",
              readyError,
            );
            ...
            try {
              ...
              for (const existing of existingRegistrations) {
                console.log(
                  `🔍 Existing registration scope: ${existing.scope}, active: ${!!existing.active}`,
                );
                if (
                  existing.active &&
                  existing.scope.includes(window.location.origin)
                ) {
              ...
              console.error(
                "❌ Failed to manually register Service Worker:",
                registerError,
              );
          ...
          if (!registration.active) {
            console.warn(
              "⚠️ Service Worker is not active, attempting to activate...",
            );
            ...
            if (updatedRegistration.active) {
              ...
              console.warn(
                "⚠️ Service Worker still not active, will try direct notification fallback",
              );
            }
          } else {
          }
          ...
          try {
            ...
            if (isMobile || isPWA) {
              console.log(
                `📱 ${isPWA ? "PWA" : "Mobile"} device detected - using optimized notification approach`,
              );
              ...
              if (registration && registration.active) {
                ...
                console.log(
                  `🎉 ${isPWA ? "PWA" : "Mobile"} Service Worker notification shown successfully!`,
                );
              } else {
                console.warn(
                  "⚠️ Service Worker not active, using direct notification for mobile",
                );
                if (
                    ...
                    if (document.hidden) {
                      try {
                        if (
                          ...
                          new Notification(
                            `${queueItem.payload.title} (Mobile)`,
                            {
                              body: queueItem.payload.body,
                              icon:
                                queueItem.payload.icon ||
              ...
              if (registration && registration.active) {
                ...
                console.warn(
                  "⚠️ Service Worker not active, using direct notification for desktop",
                );
                if (
                  "Notification" in window &&
                  Notification.permission === "granted"
                  ...
                  throw new Error(
                    "No notification method available for desktop",
                  );
                }
              }
            }
          ...
          if (isPWA && document.hidden) {
            console.error(
              "🚨 CRITICAL: PWA is minimized and Service Worker failed!",
            );
            console.error(
              "🚨 This means push notifications will NOT work until user reopens app",
            );
            ...
            try {
              ...
              if ("serviceWorker" in navigator) {
                ...
                if (activeRegistration) {
                  ...
                  console.error(
                    "❌ No active Service Worker found for recovery",
                  );
              ...
              await this.markNotificationFailed(
                queueItem.id,
                `PWA recovery failed: ${recoveryError.message}`,
              );
            }
            ...
            const handlePWAReopen = async () => {
              if (!document.hidden && isPWA) {
                ...
                try {
                  ...
                  await retryRegistration.showNotification(
                    queueItem.payload.title,
                    {
                      ...notificationOptions,
                  ...
                  console.error(
                    "❌ Delayed notification retry failed:",
                    retryError,
                  );
                }
            ...
            console.warn(
              "⚠️ Service Worker notification failed, trying direct notification fallback:",
              swError,
            );
            ...
            try {
              if (
                ...
                await this.markNotificationFailed(
                  queueItem.id,
                  `Both SW and direct notifications failed: ${swError.message}`,
                );
              }
            } catch (directError) {
              console.error(
                "❌ Direct notification fallback also failed:",
                directError,
              );
              await this.markNotificationFailed(
                queueItem.id,
                `All notification methods failed: ${swError.message} -> ${directError.message}`,
              );
            }
          }
        }
        ...
        console.warn(
          "⚠️ Direct browser notifications not supported on mobile - notification skipped",
        );
      } else {
      ...
      console.error(
        `❌ Failed to send push notification ${queueItem.id}:`,
        error,
      );
  ...
  private async markNotificationFailed(
    ...
    try {
      ...
      console.log(
        `📝 Notification ${notificationId} marked as failed: ${errorMessage}`,
      );
    } catch (error) {
      console.error(
  ...
  async cleanupOldQueueEntries(olderThanDays: number = 7) {
    // Cleanup disabled - was deleting notifications before they could be processed
    console.log(
      `🧹 [DISABLED] Old queue cleanup is disabled to prevent notification deletion`,
    );
    return;
  }
 ...
 */
export async function promptMobilePushNotifications(
  userId: string,
): Promise<boolean> {
  try {
 ...
 */
export async function checkAndPromptPushNotifications(
  userId: string,
): Promise<void> {
  try {
...
if (typeof window !== "undefined" && import.meta.env.DEV) {
  ...
  (window as any).cleanupOldNotifications = (days = 7) => {
    console.log(
      `🧹 [DISABLED] Cleanup is disabled to prevent notification deletion`,
    );
  };

  // Manual cleanup function disabled to prevent notification deletion
  (window as any).deleteDuplicateNotifications = async (
    notificationId: string,
    userId: string,
  ) => {
    console.log(
      `🗑️ [DISABLED] Manual duplicate deletion is disabled to prevent notification loss`,
    );
    return;
  };
  ...
  (window as any).findNotificationIds = async () => {
    try {
      ...
      queueItems?.forEach((item, index) => {
        console.log(
          `${index + 1}. ID: ${item.id}, Notification: ${item.notification_id}, User: ${item.user_id}, Status: ${item.status}`,
        );
      });
      ...
      if (queueItems && queueItems.length > 0) {
        ...
        console.log(
          `💡 To test deletion, use: deleteDuplicateNotifications('${first.notification_id}', '${first.user_id}')`,
        );
      } else {
      }
    } catch (error) {
  ...
  (window as any).testDatabaseTrigger = async () => {
    try {
      ...
      if (error) {
        ...
        console.error(
          "❌ This suggests the trigger function was not properly installed",
        );
      } else {
  ...
  (window as any).testExistingNotification = async () => {
    try {
      ...
      console.log(
        `🔍 Calling queue_push_notification function for notification: ${notificationId}`,
      );
  ...
  (window as any).testBasicNotification = async () => {
    try {
      ...
      if ("serviceWorker" in navigator) {
        ...
        console.warn(
          "⚠️ Service Worker not supported - notifications disabled",
        );
      }
    } catch (err) {
  ...
  (window as any).checkSubscriptionLimits = async (userId?: string) => {
    const user =
      userId || (await (window as any).persistentAuth?.getCurrentUser()?.id);
    if (!user) {
    ...
    try {
      ...
      const mobile =
        subscriptions?.filter((s) => s.device_type === "mobile") || [];
      const desktop =
        subscriptions?.filter((s) => s.device_type === "desktop") || [];
      ...
      if (mobile.length > 1) {
        console.warn(
          `⚠️ Mobile subscriptions exceed limit! Found ${mobile.length}, limit is 1`,
        );
      }
      if (desktop.length > 1) {
        console.warn(
          `⚠️ Desktop subscriptions exceed limit! Found ${desktop.length}, limit is 1`,
        );
      }
      ...
      return {
        mobile,
        desktop,
        withinLimits: mobile.length <= 1 && desktop.length <= 1,
      };
    } catch (err) {
  ...
  (window as any).testSubscriptionFallback = async (queueItemId?: string) => {
    ...
    try {
      ...
      const success = await (
        pushNotificationProcessor as any
      ).processFallbackSubscription(queueItem);
  ...
  (window as any).findOrphanedQueueItems = async () => {
    ...
    try {
      ...
      console.log(
        `🔍 Checking ${queueItems.length} queue items for orphaned subscriptions...`,
      );
      ...
      if (orphaned.length === 0) {
        console.log(
          "✅ No orphaned queue items found - all subscriptions are valid",
        );
      } else {
        console.warn(`⚠️ Found ${orphaned.length} orphaned queue items:`);
        orphaned.forEach((item, index) => {
          console.log(
            `${index + 1}. Queue ${item.queue_id} - User ${item.user_id} - ${item.issue}`,
          );
        });

        console.log(
          "💡 Run processOnce() to trigger automatic fallback processing",
        );
      }
