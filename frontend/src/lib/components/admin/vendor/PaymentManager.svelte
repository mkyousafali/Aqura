...
import { openWindow } from '$lib/utils/windowManagerUtils';
	...
	import ExpensesManager from '../finance/ExpensesManager.svelte';
	...
	$: if (scheduledPayments && receivingRecords.length > 0 && paidPayments) {
		calculateStatistics();
	}
	...
	onMount(async () => {
		...
		await loadPaidPayments(); // Load paid payments from vendor_payment_schedule
		await loadExpenseSchedulerPayments(); // Load expense scheduler payments
...
Auto-generated by frontend fallback system (${source})`,
	...
	function calculateStatistics() {
		...
		allScheduledPaymentsData.forEach(scheduleRecord => {
			...
			if (scheduleRecord.is_paid === true) {
				...
				if (scheduleRecord.due_date) {
					// Use Saudi Arabia timezone for consistent date comparison
					const saudiToday = new Date().toLocaleString('en-CA', { 
						timeZone: 'Asia/Riyadh',
						year: 'numeric',
						month: '2-digit',
					...
					const dueDate = new Date(scheduleRecord.due_date + 'T00:00:00');
					const daysDiff = Math.floor((dueDate - todaySaudi) / (1000 * 60 * 60 * 24));
		...
		paidPayments.forEach(payment => {
			...
			// Get payment method from vendor_payment_schedule
			const paymentMethod = payment.payment_method || 'Cash on Delivery';
		...
		// Calculate expense scheduler totals
		...
		stats = {
			...
			overdueAmount: overdueAmount + expenseOverdueAmount, // Include expense overdue in total
	...
	async function refreshData() {
		await loadScheduledPayments(); // Load scheduled payments first (for table logic)
		await loadAllScheduledPaymentsForCards(); // Load all scheduled payments (for cards)
		await loadPaidPayments(); // Load paid payments
		await loadTaskStatusData(); // Load task status data (for Card 4)
		await loadReceivingRecords();
			...
			<div class="status-grid">
				<div class="status-card clickable scheduled-card" on:click={openScheduledPayments}>
					...
					<div class="card-content">
						...
						<p class="total-amount">{formatCurrency(stats.scheduledAmount + (stats.expenseScheduledAmount || 0))}</p>
						<div class="card-subsection">
							<div class="subsection-header">Vendor Payments: {formatCurrency(stats.scheduledAmount)}</div>
						...
						{#if stats.expenseScheduledCount > 0}
							<div class="card-subsection expense-section">
								<div class="subsection-header" style="color: #dc2626;">Expenses: {formatCurrency(stats.expenseScheduledAmount)}</div>
								<p class="count-label" style="color: #dc2626;">{stats.expenseScheduledCount} expense{stats.expenseScheduledCount !== 1 ? 's' : ''} scheduled</p>
							</div>
						{/if}
				...
				<div class="status-card paid-card clickable" on:click={openPaidPaymentsModal}>
					...
					<div class="card-content">
						<h3>Paid Payments</h3>
						<p class="total-amount">{formatCurrency(stats.paidAmount + (stats.expensePaidAmount || 0))}</p>
						<div class="card-subsection">
							<div class="subsection-header">Vendor Payments: {formatCurrency(stats.paidAmount)}</div>
							<div class="payment-breakdown">
						...
						{#if stats.expensePaidCount > 0}
							<div class="card-subsection expense-section">
								<div class="subsection-header" style="color: #059669;">Expenses Paid: {formatCurrency(stats.expensePaidAmount)}</div>
								<p class="count-label" style="color: #059669;">{stats.expensePaidCount} expense{stats.expensePaidCount !== 1 ? 's' : ''} paid</p>
							</div>
						{/if}
						<p class="click-hint">Click to view details</p>
					</div>
				...
				<div class="status-card unpaid-scheduled-card clickable" on:click={openUnpaidScheduledModal}>
					...
					<div class="card-content">
						...
						<p class="total-amount">{formatCurrency(stats.unpaidScheduledAmount + (stats.expenseUnpaidAmount || 0))}</p>
						<div class="card-subsection">
							<div class="subsection-header">Vendor Payments: {formatCurrency(stats.unpaidScheduledAmount)}</div>
							<div class="payment-breakdown">
						...
						{#if stats.expenseUnpaidCount > 0}
							<div class="card-subsection expense-section">
								<div class="subsection-header" style="color: #dc2626;">Expenses Unpaid: {formatCurrency(stats.expenseUnpaidAmount)}</div>
								<p class="count-label" style="color: #dc2626;">{stats.expenseUnpaidCount} expense{stats.expenseUnpaidCount !== 1 ? 's' : ''} unpaid</p>
							</div>
						{/if}
						<p class="click-hint">Click to view details</p>
					</div>
				...
				<div class="status-card overdue-card clickable" on:click={openOverdueModal}>
					...
					<div class="card-content">
						<h3>Overdue</h3>
						<p class="total-amount">{formatCurrency(stats.overdueAmount)}</p>
						<div class="payment-breakdown">
							{#each Object.entries(stats.overdueByPaymentMethod || {}) as [method, amount]}
								<div class="payment-method-item">
									<span class="method-name">{method}:</span>
									<span class="method-amount">{formatCurrency(amount)}</span>
								</div>
						...
						<p class="count-label">{stats.overdueBills} bills (Overdue & Due in 5 days)</p>
						{#if stats.expenseOverdueCount > 0}
							<div class="card-subsection expense-section">
								<div class="subsection-header" style="color: #dc2626;">Expenses Overdue: {formatCurrency(stats.expenseOverdueAmount)}</div>
								<p class="count-label" style="color: #dc2626;">{stats.expenseOverdueCount} expense{stats.expenseOverdueCount !== 1 ? 's' : ''} overdue</p>
							</div>
						{/if}
						<p class="click-hint">Click to view details</p>
					</div>
