			...
			<div class="bg-white/40 backdrop-blur-xl rounded-[2.5rem] border border-white shadow-[0_32px_64px_-16px_rgba(0,0,0,0.08)] p-8 h-full flex flex-col overflow-hidden">
				...
				<div class="flex flex-wrap items-center gap-2 mb-4">
					<div class="relative flex-1 min-w-[180px] max-w-[320px]">
						...
						<input type="text" bind:value={searchQuery} on:input={triggerReload} placeholder={$locale === 'ar' ? 'بحث بالاسم، الفرع، الباركود...' : 'Search name, branch, barcode...'} class="w-full {$locale === 'ar' ? 'pr-9 pl-3' : 'pl-9 pr-3'} py-2 bg-white border border-slate-200 rounded-xl text-xs text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition-all" />
					</div>
					<select bind:value={filterStatus} on:change={triggerReload} class="px-3 py-2 bg-white border border-slate-200 rounded-xl text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-400 cursor-pointer min-w-[110px]">
					...
					<div class="flex items-center gap-1">
						...
						<input type="date" bind:value={filterDateFrom} on:change={triggerReload} class="px-2 py-1.5 bg-white border border-slate-200 rounded-xl text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-400 cursor-pointer" />
					</div>
					<div class="flex items-center gap-1">
						<span class="text-[10px] text-slate-400 font-bold">{$locale === 'ar' ? 'إلى' : 'To'}</span>
						<input type="date" bind:value={filterDateTo} on:change={triggerReload} class="px-2 py-1.5 bg-white border border-slate-200 rounded-xl text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-400 cursor-pointer" />
					</div>
					{#if hasActiveFilters}
					...
					{#if totalRequests > PAGE_SIZE}
						<div class="flex items-center justify-between pt-4 border-t border-slate-200 mt-2">
							<span class="text-xs text-slate-500">
								{$locale === 'ar' ? 'عرض' : 'Showing'} {currentPage * PAGE_SIZE + 1}–{Math.min((currentPage + 1) * PAGE_SIZE, totalRequests)} {$locale === 'ar' ? 'من' : 'of'} {totalRequests}
							</span>
							<div class="flex gap-2">
								<button on:click={() => loadRequests(currentPage - 1)} disabled={currentPage === 0 || loading}
								...
								<button on:click={() => loadRequests(currentPage + 1)} disabled={(currentPage + 1) * PAGE_SIZE >= totalRequests || loading}
									class="px-3 py-1.5 text-xs font-bold rounded-lg border border-slate-300 text-slate-600 hover:bg-slate-100 disabled:opacity-40 disabled:cursor-not-allowed">
