...
returns table(source_table text,source_record_id text,branch_id text,vendor_name text,bill_number text,amount numeric,requested_at timestamptz,status text)
language sql security definer set search_path=public as $$
  select distinct q.* from (
    select 'receiving_records'::text source_table,r.id::text source_record_id,r.branch_id::text branch_id,coalesce(v.vendor_name,'Unknown vendor') vendor_name,r.bill_number::text bill_number,coalesce(r.final_bill_amount,r.bill_amount,0)::numeric amount,r.autotask_advance_approval_requested_at requested_at,r.autotask_advance_approval_status status
    ...
    union all
    select 'pending_receiving_records'::text,r.id::text,r.branch_id::text,coalesce(v.vendor_name,'Unknown vendor'),r.bill_number::text,coalesce(r.final_bill_amount,r.bill_amount,0)::numeric,r.autotask_advance_approval_requested_at,r.autotask_advance_approval_status
