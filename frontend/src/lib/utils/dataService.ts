...
import { db, offline, supabase, type Employee, type Branch, type Vendor, type User } from './supabase';
...
export const dataService = {
	...
	hrDepartments: {
		...
		async create(department: { department_name_en: string; department_name_ar: string }): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					const { data, error } = await supabase
						.from('hr_departments')
				...
				await delay(MOCK_DATA_DELAY);
				return { data: { id: Date.now(), ...department, created_at: new Date().toISOString() }, error: null };
			}
		},

		async update(id: string, updates: Partial<{ department_name_en: string; department_name_ar: string }>): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					const { data, error } = await supabase
						.from('hr_departments')
	...
	hrLevels: {
		...
		async create(level: { level_name_en: string; level_name_ar: string; level_order: number }): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					const { data, error } = await supabase
						.from('hr_levels')
				...
				await delay(MOCK_DATA_DELAY);
				return { data: { id: Date.now(), ...level, created_at: new Date().toISOString() }, error: null };
			}
		},

		async update(id: string, updates: Partial<{ level_name_en: string; level_name_ar: string; level_order: number }>): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					const { data, error } = await supabase
						.from('hr_levels')
	...
	hrPositions: {
		...
		async create(position: { position_title_en: string; position_title_ar: string; department_id: string; level_id: string; description?: string }): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					const { data, error } = await supabase
						.from('hr_positions')
				...
				await delay(MOCK_DATA_DELAY);
				return { data: { id: Date.now(), ...position, created_at: new Date().toISOString() }, error: null };
			}
		},

		async update(id: string, updates: Partial<{ position_title_en: string; position_title_ar: string; department_id: string; level_id: string; description: string }>): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					const { data, error } = await supabase
						.from('hr_positions')
	...
	hrEmployees: {
		...
		async getByBranch(branchId: string): Promise<{ data: any[] | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					...
					if (assignments && assignments.length > 0) {
						const positionIds = [...new Set(assignments.map(a => a.position_id))];
						...
						console.log('Departments query result:', { departmentsData, departmentsError });
					...
					const enrichedEmployees = employees.map(employee => {
						...
						const position = assignment ? positions.find(p => p.id === assignment.position_id) : null;
						const department = assignment ? departments.find(d => d.id === assignment.department_id) : null;
						
						console.log(`Employee ${employee.name}:`, { assignment, position, department });
						
						return {
							...employee,
							department: department?.department_name_en || department?.department_name_ar || null,
							position: position?.position_title_en || position?.position_title_ar || null,
		...
		async create(employee: { employee_id: string; name: string; branch_id: string; hire_date?: string; status?: string }): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					const { data, error } = await supabase
						.from('hr_employees')
				...
				await delay(MOCK_DATA_DELAY);
				return { data: { id: Date.now(), ...employee, created_at: new Date().toISOString() }, error: null };
			}
		},

		async syncEmployee(employeeId: string, hrCode: string, name: string, branchId?: string, positionTitle?: string): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					const { data, error } = await supabase
						.rpc('sync_employee_with_hr', {
	...
	hrReporting: {
		...
		async create(reportingMap: { subordinate_position_id: string; manager_positions: string[] }): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					...
					if (reportingMap.manager_positions[0]) insertData.manager_position_1 = reportingMap.manager_positions[0];
					if (reportingMap.manager_positions[1]) insertData.manager_position_2 = reportingMap.manager_positions[1];
					if (reportingMap.manager_positions[2]) insertData.manager_position_3 = reportingMap.manager_positions[2];
					if (reportingMap.manager_positions[3]) insertData.manager_position_4 = reportingMap.manager_positions[3];
					if (reportingMap.manager_positions[4]) insertData.manager_position_5 = reportingMap.manager_positions[4];
				...
				await delay(MOCK_DATA_DELAY);
				return { data: { id: Date.now(), ...reportingMap, created_at: new Date().toISOString() }, error: null };
			}
		},

		async update(id: string, updates: { manager_positions: string[] }): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					...
					if (updates.manager_positions[0]) updateData.manager_position_1 = updates.manager_positions[0];
					if (updates.manager_positions[1]) updateData.manager_position_2 = updates.manager_positions[1];
					if (updates.manager_positions[2]) updateData.manager_position_3 = updates.manager_positions[2];
					if (updates.manager_positions[3]) updateData.manager_position_4 = updates.manager_positions[3];
					if (updates.manager_positions[4]) updateData.manager_position_5 = updates.manager_positions[4];
	...
	hrAssignments: {
		...
		async create(assignment: { employee_id: string; position_id: string; department_id: string; level_id: string; branch_id: string; effective_date?: string }): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					...
					const { data, error } = await supabase
						...
						.insert([{
							...
							effective_date: assignment.effective_date || new Date().toISOString().split('T')[0],
							is_current: true,
							created_at: new Date().toISOString()
						}])
						.select(`
				...
				await delay(MOCK_DATA_DELAY);
				return { data: { id: Date.now(), ...assignment, created_at: new Date().toISOString() }, error: null };
			}
		},

		async update(id: string, updates: Partial<{ position_id: string; department_id: string; level_id: string; effective_date: string }>): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					const { data, error } = await supabase
						.from('hr_position_assignments')
	...
	hrFingerprint: {
		...
		async create(transaction: { employee_id: string; name?: string; branch_id: number; transaction_date: string; transaction_time: string; punch_state: string; device_id?: string }): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					const { data, error } = await supabase
						.from('hr_fingerprint_transactions')
				...
				await delay(MOCK_DATA_DELAY);
				return { data: { id: Date.now(), ...transaction, created_at: new Date().toISOString() }, error: null };
			}
		},

		async createMany(transactions: any[]): Promise<{ data: any[] | null; error: any; successCount: number; errorCount: number; errors: string[] }> {
			if (USE_SUPABASE) {
				let successCount = 0;
				let errorCount = 0;
				const errors = [];
				...
				await delay(MOCK_DATA_DELAY);
				return { data: transactions, error: null, successCount: transactions.length, errorCount: 0, errors: [] };
			}
		}
	},
	...
	hrContacts: {
		...
		async getByBranch(branchId: string): Promise<{ data: any[] | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					...
					console.error('Failed to fetch HR employee contacts by branch:', error);
					return { data: null, error: error.message };
				}
			} else {
				await delay(MOCK_DATA_DELAY);
		...
		async create(contact: { employee_id: string; whatsapp_number?: string; contact_number?: string; email?: string }): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					const { data, error } = await supabase
						.from('hr_employee_contacts')
				...
				await delay(MOCK_DATA_DELAY);
				return { data: { id: Date.now(), ...contact, created_at: new Date().toISOString() }, error: null };
			}
		},

		async update(id: string, updates: { whatsapp_number?: string; contact_number?: string; email?: string }): Promise<{ data: any | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					const { data, error } = await supabase
						.from('hr_employee_contacts')
	...
	hrDocuments: {
		...
		async getByEmployeeId(employeeId: string): Promise<{ data: any[] | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					const { data, error } = await supabase
						.from('hr_employee_documents')
		...
		async getByBranch(branchId: number): Promise<{ data: any[] | null; error: any }> {
			if (USE_SUPABASE) {
				try {
					...
					const branchDocuments = data?.filter(doc => doc.employee?.branch_id === branchId) || [];
		...
		async getExpiringSoon(days: number = 30): Promise<{ data: any[] | null; error: any }> {
			if (USE_SUPABASE) {
				try {
