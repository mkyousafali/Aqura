-- Migration 70: Fix Cash-on-Delivery Auto-Payment
-- Description: Updates the payment automation trigger to automatically mark cash-on-delivery payments as paid
-- Date: 2025-10-23
-- Version: 1.0.11
--
-- PROBLEM: Cash-on-delivery payments are not being automatically marked as paid
-- SOLUTION: Update trigger to handle INSERT operations and auto-mark COD payments
--
-- FEATURES:
-- ✅ Auto-mark cash-on-delivery payments as paid on INSERT
-- ✅ Create transaction/task/notification when marked as paid (INSERT or UPDATE)
-- ✅ Use accountant from receiving_records table
-- ✅ Custom task title: "New payment made — enter into the ERP, update the ERP reference, and upload the payment receipt"
-- ✅ Enhanced notifications with bill details
...
-- Create the comprehensive automation function
...
COMMENT ON FUNCTION auto_create_payment_transaction_and_task() IS 'FIXED v1.0.11: Auto-marks cash-on-delivery as paid on INSERT, creates tasks/transactions/notifications for all paid payments';
COMMENT ON TRIGGER trigger_auto_create_payment_transaction_and_task ON vendor_payment_schedule IS 'Triggers on INSERT (COD auto-payment) and UPDATE (manual payment marking)';

-- Success message
DO $$
BEGIN
    RAISE NOTICE '✅ Migration 70 applied successfully!';
    RAISE NOTICE '   - Cash-on-delivery payments will now be auto-marked as paid';
    RAISE NOTICE '   - Payment transactions and tasks will be created automatically';
    RAISE NOTICE '   - Accountants will receive notifications with 24-hour deadline';
END $$;
