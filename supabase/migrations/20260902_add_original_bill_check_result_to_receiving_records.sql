-- Persists the AI "Check" verdict on the Original Bill document (Vendor Name / Bill
-- Amount incl. VAT / Bill Date extracted by Gemini, plus the AI's own match judgment
-- against the record) so it survives reloads — mirrors the existing erp_check_result
-- column/pattern. Written by ReceivingRecords.svelte when "Done" is pressed in the
-- AI Bill Check popup (after an initial Check or a Recheck).

ALTER TABLE public.receiving_records
    ADD COLUMN IF NOT EXISTS original_bill_check_result jsonb;

COMMENT ON COLUMN public.receiving_records.original_bill_check_result IS
    'AI-extracted Vendor Name / Bill Amount incl. VAT / Bill Date from the Original Bill document, plus the AI''s own match verdicts against the record. Saved when the user presses Done in the AI Bill Check popup. Shape: {vendorName, billAmountIncludingVat, billDate, vendorNameMatches, billAmountMatches, billDateMatches, status: "matched"|"mismatch", checkedAt}.';

-- Re-expose the new column through get_receiving_records_with_details. Same 10-arg
-- signature as 20260901_add_erp_check_status_filter_to_receiving_records.sql — only
-- the return shape changes (one extra column), which is why this is DROP + CREATE
-- rather than CREATE OR REPLACE (Postgres refuses to change a function's return type
-- in place).
DROP FUNCTION IF EXISTS public.get_receiving_records_with_details(integer, integer, text, text, text, text, text, date, date, text);
...
CREATE FUNCTION public.get_receiving_records_with_details(
    ...
    p_erp_check_status_filter text DEFAULT NULL
) RETURNS TABLE(
    ...
    pr_excel_verified_date timestamptz, erp_check_result jsonb,
    original_bill_check_result jsonb, total_count bigint
...
BEGIN
    ...
    WHERE (p_branch_id IS NULL OR r.branch_id::text = p_branch_id)
      ...
      AND (p_erp_check_status_filter IS NULL OR p_erp_check_status_filter = ''
           OR (p_erp_check_status_filter = 'issues' AND r.erp_check_result->>'status' IN ('mismatch', 'not_found'))
           OR (p_erp_check_status_filter != 'issues' AND r.erp_check_result->>'status' = p_erp_check_status_filter));
    ...
    SELECT r.id::text, r.bill_number::text, r.vendor_id::text, r.branch_id::text,
        ...
        ps.pr_excel_verified_date, r.erp_check_result, r.original_bill_check_result, v_total
    FROM receiving_records r
...
GRANT EXECUTE ON FUNCTION public.get_receiving_records_with_details(integer, integer, text, text, text, text, text, date, date, text)
