<script lang="ts">
	...
	// AQ-SEC-005: this screen no longer talks to system_api_keys directly — it's the
	// only table where the anon key could read every credential, confirmed exploitable.
	// All reads/writes now go through /api/system-api-keys, which uses the service key.
	let apiKeys: any[] = [];
	let loading = true;
	...
	async function toggleActive(key: any) {
		const res = await fetch('/api/system-api-keys', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ action: 'toggle', id: key.id, is_active: !key.is_active })
		});
		const result = await res.json();
		if (result.success) key.is_active = !key.is_active;
		apiKeys = [...apiKeys];
	}
	...
	async function addNewKey() {
		...
		const res = await fetch('/api/system-api-keys', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ action: 'insert', ...newService })
		});
		const result = await res.json();
		if (result.success) {
	...
	async function deleteKey(key: any) {
		...
		await fetch('/api/system-api-keys', {
			method: 'POST',
			headers: { 'Content-Type': 'application/json' },
			body: JSON.stringify({ action: 'delete', id: key.id })
		});
		await loadKeys();
	}
