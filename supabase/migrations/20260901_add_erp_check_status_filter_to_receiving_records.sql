-- Adds a filter for the ERP Check column: p_erp_check_status_filter = 'issues' returns rows
-- whose persisted erp_check_result.status is 'mismatch' or 'not_found' (the "needs attention"
-- set), powering a one-click filter button in ReceivingRecords.svelte. Other exact status
-- values ('matched'/'mismatch'/'not_found') are also accepted for flexibility.
