<script lang="ts">
	...
	import { FUNCTION_CODES } from '$lib/utils/permissions';
	...
	async function loadUsers() {
		try {
			...
			const { data, error: err } = await supabase
				...
				.select('id, username, role_type, employee_name, branch_name, status')
				.order('username');
	...
	async function selectUser(userId: string) {
		try {
			...
			if (data && data.length > 0) {
				functionPermissions = data as FunctionWithPermissions[];
			} else {
				// If no permissions exist, create empty permissions for all functions
				const { data: functions, error: funcErr } = await supabase
					.from('app_functions')
					.select('id, function_code, function_name, category')
					.eq('is_active', true)
					.order('category', { ascending: true });
	...
	function updatePermission(functionCode: string, action: string, value: boolean) {
		...
		if (perm) {
			...
			functionPermissions = functionPermissions; // Trigger reactivity
		}
	}
	...
	async function savePermissions() {
		...
		try {
			...
			error = 'Failed to save permissions: ' + (err instanceof Error ? err.message : 'Unknown error');
		} finally {
			saving = false;
		}
	}
...
{:else}
	<div class="permission-manager">
		...
		<div class="header">
			...
			<p>Assign specific permissions to users for different system functions</p>
		</div>
