...
<div class="page" dir={isAr ? 'rtl' : 'ltr'} style={layout?.main_layout?.bg_color ? `background: ${layout.main_layout.bg_color}` : undefined}>
	...
	<!-- Hero: Access Code (customer sign up / login) -->
	...
	<!-- Home content (from main login page) -->
	<section class="hero home-content">
...
<style>
	...
	.customer-login-card {
		...
		max-width: 480px;
		background: #ffffff;
		border: 1px solid rgba(15, 61, 47, 0.06);
		border-radius: 22px;
		padding: 2.25rem 2rem;
		box-shadow: 0 30px 70px rgba(15, 61, 47, 0.14), 0 4px 12px rgba(15, 61, 47, 0.05);
		overflow: hidden;
	}
	...
	.customer-login-card::before {
		...
		right: 0;
		height: 5px;
		background: linear-gradient(90deg, #0e7c3f 0%, #15A34A 50%, #c8912f 100%);
	}
