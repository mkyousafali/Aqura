...
// AQ-SEC-003: the bridge secret comes from erp_connections.bridge_api_secret per branch.
// No hardcoded fallback — a branch missing its secret is simply skipped (see below).
...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		const { data: erpConfigs, error: configError } = await supabase
			...
			.select('tunnel_url, erp_branch_id, branch_id, branch_name, bridge_api_secret, branches(id, location_en, location_ar)')
			.eq('is_active', true)
			.order('branch_id');
		...
		// This handles ERP storing "96XXXXXXXXX" vs Aqura "966XXXXXXXXX"
		...
		const branchPromises = erpConfigs.map(async (config: any) => {
			if (!config?.tunnel_url || !config?.bridge_api_secret) return;
			...
			try {
				...
				const response = await fetch(`${baseUrl}/query`, {
					...
					headers: {
						...
						'x-api-secret': config.bridge_api_secret
					},
