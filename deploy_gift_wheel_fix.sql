...
BEGIN
    ...
    -- Pick random reward based on weight
    random_val := floor(random() * total_weight)::integer;
    cumulative := 0;
    ...
    FOREACH reward_row IN ARRAY eligible_rewards LOOP
        cumulative := cumulative + reward_row.weight;
        IF random_val < cumulative THEN
            -- Found the winning reward
            
            -- Update reward usage
            UPDATE gift_wheel_rewards SET usage_count = usage_count + 1 WHERE id = reward_row.id;

            -- Insert the winning spin
            INSERT INTO gift_wheel_spins (bill_number, bill_amount, bill_date, bill_image_url, reward_id, reward_label, reward_label_ar, reward_label_en, coupon_code, is_winner)
            VALUES (p_bill_number, p_bill_amount, p_bill_date, p_bill_image_url, reward_row.id, reward_row.label, reward_row.label_ar, reward_row.label_en, reward_row.coupon_code, true)
            RETURNING * INTO existing_spin;

            RETURN jsonb_build_object(
                'success', true,
                'is_winner', true,
                'reward_id', existing_spin.reward_id,
                'reward_label', existing_spin.reward_label,
                'reward_label_ar', existing_spin.reward_label_ar,
                'reward_label_en', existing_spin.reward_label_en,
                'coupon_code', existing_spin.coupon_code,
                'max_discount', reward_row.max_discount,
                'expiry_date', reward_row.expiry_date,
                'reward_value', reward_row.value
            );
        END IF;
    END LOOP;

    -- If we get here, return no reward
    INSERT INTO gift_wheel_spins (bill_number, bill_amount, bill_date, bill_image_url, reward_id, is_winner)
    VALUES (p_bill_number, p_bill_amount, p_bill_date, p_bill_image_url, NULL, false)
    RETURNING * INTO existing_spin;
