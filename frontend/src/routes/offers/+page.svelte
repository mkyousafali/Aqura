<script lang="ts">
	...
	onMount(() => {
		...
		// This route is the "offers" entry point of the shared login page - jump straight to
		// the Offers section instead of showing the Home section first.
		requestAnimationFrame(() => {
			document.getElementById('offers')?.scrollIntoView({ behavior: 'auto' });
		});
	...
	let selectedOfferBranchId: number | null = null;
	$: offerBranchOptions = Array.from(
		new Map(validOffers.filter((o: any) => o.branch).map((o: any) => [o.branch.id, o.branch])).values()
	);
	$: filteredOffers = selectedOfferBranchId
		? validOffers.filter((o: any) => o.branch_id === selectedOfferBranchId)
		: validOffers;
	$: offersVisible = validOffers.length > 0;
	$: useMarqueeOffers = filteredOffers.length > 4;
	$: offerBranchFilterPlaceholder = isAr ? 'كل الفروع' : 'All Branches';
	$: offersHeadingText =
...
<div class="page" dir={isAr ? 'rtl' : 'ltr'} style={layout?.main_layout?.bg_color ? `background: ${layout.main_layout.bg_color}` : undefined}>
	...
	{#if offersVisible}
		<section class="offers" id="offers">
			...
			{/if}
			{#if offerBranchOptions.length > 1}
				<div class="offer-branch-filter">
					<select bind:value={selectedOfferBranchId} aria-label={offerBranchFilterPlaceholder}>
						<option value={null}>{offerBranchFilterPlaceholder}</option>
						{#each offerBranchOptions as b (b.id)}
							<option value={b.id}>{getBranchDisplayLabel(b)}</option>
						{/each}
					</select>
				</div>
			{/if}
			...
			>
				{#each (useMarqueeOffers ? [...filteredOffers, ...filteredOffers] : filteredOffers) as offer, i (offer.id + '-' + i)}
