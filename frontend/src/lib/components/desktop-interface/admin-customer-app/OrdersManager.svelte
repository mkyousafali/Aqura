<script lang="ts">
  ...
  function loadPrintConfig() {
    try {
      const saved = localStorage.getItem('aqura_print_config');
      if (saved) {
  ...
  function savePrintConfig() {
    try {
      localStorage.setItem('aqura_print_config', JSON.stringify({
  ...
  function playNotificationSound(orderNumber?: string, totalQuantity?: number) {
    ...
    // AQ-SEC-005: no longer reads a build-time VITE_ key baked into the shipped bundle —
    // the key is looked up server-side by /api/google-ai-proxy.
    if (orderNumber) {
      ...
      playTTS(textEn, 'en-US', 'en-US-Standard-D', 1.0)
        .then(() => playTTS(textAr, 'ar-XA', 'ar-XA-Standard-A', 1.0))
        .catch(() => {
          playFallbackSound();
        });
    } else {
  ...
  async function playTTS(text: string, langCode: string, voiceName: string, volume: number = 1.0) {
    const response = await fetch('/api/google-ai-proxy', {
      method: 'POST',
