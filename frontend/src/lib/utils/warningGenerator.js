...
export async function generateWarningClientSide(assignment, language = 'en') {
	console.warn('Using client-side API call - this is not recommended for production');
	...
	throw new Error('Client-side API calls are not implemented for security reasons');
}
...
export async function generateWarning(assignment, language = 'en') {
	try {
		...
		if (error.message.includes('404') || error.message.includes('not found')) {
			console.warn('Server-side API not available, falling back to alternative method');
			// In a real implementation, you would either:
			// 1. Use a serverless function service
			// 2. Use a third-party API proxy service
			// 3. Have a separate backend service
			throw new Error('Warning generation service not available. Please contact administrator.');
		}
		throw error;
	}
}
...
export function createWarningPrompts() {
	return {
		english: (assignment) => `Generate a professional performance warning notice for an employee with poor task completion performance.
...
Generate a formal, professional warning letter content. Do NOT include dates, names, or company placeholders.`,
		...
		arabic: (assignment) => `أنشئ محتوى إشعار تحذير مهني لموظف لديه أداء ضعيف في إنجاز المهام.
