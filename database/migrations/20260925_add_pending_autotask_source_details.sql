...
DROP FUNCTION IF EXISTS public."Autotask_list_pending_tasks"(uuid);

CREATE FUNCTION public."Autotask_list_pending_tasks"(p_requesting_user_id uuid)
RETURNS TABLE(
  id uuid, task_number integer, rule_code text, title_en text, title_ar text,
  status text, is_overdue boolean, triggered_at timestamptz, available_at timestamptz,
  due_at timestamptz, branch_id text, branch_name_en text, branch_name_ar text,
  assignee_user_id uuid, assignee_username text, assignee_name_en text,
  assignee_name_ar text, source_table text, source_record_id text, source_refs jsonb,
  pending_dependencies jsonb, source_vendor_name text, source_bill_amount numeric,
  ...
  LEFT JOIN LATERAL (
    SELECT jsonb_agg(
      jsonb_build_object(
        ...
        'title_en',prerequisite.title_en,'title_ar',prerequisite.title_ar,
        'status',prerequisite.status,'assignee_user_id',prerequisite.assignee_user_id,
