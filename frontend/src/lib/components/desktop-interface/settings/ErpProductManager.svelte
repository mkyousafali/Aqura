<script lang="ts">
	...
	async function testConnection() {
		// AQ-SEC-003: testing now always uses the SAVED connection's branch id — the bridge
		// secret is looked up server-side from erp_connections, never sent to the browser.
		// This means a not-yet-saved tunnel URL edit can no longer be tested directly; save
		// the connection first, then test it.
		if (!selectedConfig?.branch_id) {
			connectionStatus = { success: false, message: 'Save this connection before testing it' };
			return;
		}
	...
	async function syncProducts() {
		if (!selectedConfig?.branch_id) {
			syncStatus = { success: false, message: 'Save this connection before syncing' };
			return;
		}
