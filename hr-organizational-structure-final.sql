-- =====================================================
-- FIXED ORGANIZATIONAL STRUCTURE - NO SUBQUERY ERRORS
-- =====================================================
...
-- Departments (8 total) - Using ON CONFLICT to prevent duplicates
...
-- All Positions with Reporting Structure (30 positions) - Using LIMIT 1 to prevent subquery errors
...
('Quality Assurance Manager', 'مدير ضمان الجودة', 
 (SELECT id FROM hr_departments WHERE department_name_en = 'Quality & Compliance'), 
 (SELECT id FROM hr_levels WHERE level_order = 4)),
...
('Branch Manager', 'مدير الفرع', 
 (SELECT id FROM hr_departments WHERE department_name_en = 'Branch Operations'), 
 (SELECT id FROM hr_levels WHERE level_order = 5)),
...
('Cheese Department Head', 'رئيس قسم الجبن', 
 (SELECT id FROM hr_departments WHERE department_name_en = 'Store Departments'), 
 (SELECT id FROM hr_levels WHERE level_order = 6)),
...
('Night Supervisors', 'مشرفو الليل', 
 (SELECT id FROM hr_departments WHERE department_name_en = 'Store Supervisors'), 
 (SELECT id FROM hr_levels WHERE level_order = 7)),
...
-- Level 8: Store Staff
('Accountant', 'محاسب', 
 (SELECT id FROM hr_departments WHERE department_name_en = 'Store Staff'), 
 (SELECT id FROM hr_levels WHERE level_order = 8)),
('Driver', 'سائق', 
 (SELECT id FROM hr_departments WHERE department_name_en = 'Store Staff'), 
 (SELECT id FROM hr_levels WHERE level_order = 8)),
('Cashiers', 'أمين صندوق', 
...
-- =====================================================
-- POSITION REPORTING TEMPLATE - FIXED COLUMN NAMES
-- =====================================================

-- Clear existing template data first
DELETE FROM hr_position_reporting_template;

-- Insert complete reporting structure with correct column names
...
-- Check for duplicate position names (this might cause the subquery error)
SELECT position_title_en, COUNT(*) as count 
FROM hr_positions 
GROUP BY position_title_en 
HAVING COUNT(*) > 1;
