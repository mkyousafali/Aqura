-- COMPREHENSIVE UPDATED Migration 59: Advanced Payment Automation
-- VERSION: Complete with ALL Requirements
-- 
...
-- - Payment method "cash-on-delivery", "cash on delivery", "cod", or "cash" triggers auto-payment
-- - Sets is_paid = true, paid_date = now(), payment_status = 'paid'
-- - Creates payment transaction, task, and notification immediately
--
-- REGULAR PAYMENT BEHAVIOR:
-- - Manual marking as paid (is_paid = true) triggers automation
-- - Creates payment transaction, task, and notification
--
-- ACCOUNTANT ASSIGNMENT:
-- - Primary: Uses accountant_user_id from receiving_records table
-- - Fallback: Searches for accountant/manager/admin users if no accountant in receiving record
-- - Notification sent to assigned accountant
...
-- Create comprehensive automation function
...
BEGIN
    ...
    IF v_should_auto_mark_paid OR ((OLD.is_paid IS FALSE OR OLD.is_paid IS NULL) AND NEW.is_paid IS TRUE) THEN
        ...
        -- Get receiver user details for task description
        -- Note: Using username as display_name since full_name column may not exist
        SELECT id, username, username as display_name 
        INTO v_receiver_user
        FROM users 
        WHERE id = v_receiving_record.user_id;
        
        -- Get accountant user details for notifications
        -- Note: Using username as display_name since full_name column may not exist
...
COMMENT ON FUNCTION auto_create_payment_transaction_and_task() IS 'COMPREHENSIVE payment automation: Auto-marks cash-on-delivery as paid, uses accountant from receiving records, creates tasks with specific format, sends notifications';
COMMENT ON TRIGGER trigger_auto_create_payment_transaction_and_task ON vendor_payment_schedule IS 'Triggers comprehensive payment automation on INSERT (for cash-on-delivery) and UPDATE (when manually marked as paid)';
