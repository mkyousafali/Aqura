            ...
            {#if characters.length > 0}
                <div class="bg-white/80 backdrop-blur-xl rounded-2xl border border-white shadow-[0_4px_16px_-4px_rgba(0,0,0,0.08)] p-4 space-y-2" data-char-dropdown>
                    ...
                    <button type="button" on:click={toggleCharDropdown}
                        class="w-full flex items-center justify-between gap-2 px-3 py-2.5 bg-white border border-slate-200 rounded-xl text-sm font-semibold hover:border-orange-400 transition-all">
                        <span class="flex items-center gap-2 min-w-0 flex-1">
                            {#if selectedCharacterIds.size === 0}
                                <span class="text-slate-400 text-xs">{$locale === 'ar' ? '— اختر شخصية أو أكثر —' : '— Select character(s) —'}</span>
                            {:else}
                                <span class="inline-flex items-center justify-center w-5 h-5 rounded-full bg-orange-500 text-white text-[10px] font-black flex-shrink-0">{selectedCharacterIds.size}</span>
                        ...
                        <div class="bg-white border border-slate-200 rounded-xl shadow-md max-h-72 overflow-y-auto">
                            {#each characters as char}
                                ...
                                <button type="button" on:click={() => toggleCharacter(char.id)}
                                    class="w-full flex items-center gap-2.5 p-2.5 text-left hover:bg-orange-50 border-b border-slate-100 last:border-b-0 transition-colors">
                                    <input type="checkbox" checked={isSelected} readonly
                                        class="w-4 h-4 rounded border-2 border-slate-300 text-orange-600 focus:ring-orange-500 pointer-events-none flex-shrink-0"/>
