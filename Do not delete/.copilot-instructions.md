...
**CRITICAL: Every time you commit and push changes to git, you MUST update the version number for the interface you modified.**
...
1. **Before committing**, run the appropriate version update command:
   
   **Desktop Interface Changes:**
   ```powershell
   node scripts/update-version.js desktop      # Increment by 1 (AQ1.1.1.1 → AQ2.1.1.1)
   node scripts/update-version.js desktop 5    # Increment by 5 (AQ1.1.1.1 → AQ6.1.1.1)
   ```

   **Mobile Interface Changes:**
   ```powershell
   node scripts/update-version.js mobile       # Increment by 1 (AQ1.1.1.1 → AQ1.2.1.1)
   node scripts/update-version.js mobile 3     # Increment by 3 (AQ1.1.1.1 → AQ1.4.1.1)
   ```

   **Cashier Interface Changes:**
   ```powershell
   node scripts/update-version.js cashier      # Increment by 1 (AQ1.1.1.1 → AQ1.1.2.1)
   ```

   **Customer Interface Changes:**
   ```powershell
   node scripts/update-version.js customer     # Increment by 1 (AQ1.1.1.1 → AQ1.1.1.2)
   ```

   **All Interfaces Changed:**
   ```powershell
   node scripts/update-version.js all          # Increment all by 1 (AQ1.1.1.1 → AQ2.2.2.2)
   node scripts/update-version.js all 2        # Increment all by 2 (AQ1.1.1.1 → AQ3.3.3.3)
   ```

   This will automatically:
   - Increment the appropriate interface version
   - Update root `package.json`
   - Update `frontend/package.json`
   - Update version display in `frontend/src/lib/components/desktop-interface/common/Sidebar.svelte`
   - Display the interface that was updated

2. **Update version popup content manually** (REQUIRED):
   - Edit `frontend/src/lib/components/desktop-interface/common/Sidebar.svelte` (version popup section)
   - Add feature descriptions matching your changes
   - Use specific action words: "Fixed", "Implemented", "Enhanced", "Added", "Updated"
   - Include which interface was updated in the description

3. **Then proceed with git operations**:
   ```powershell
   git add -A
   git commit -m "chore(desktop): bump version to AQ2.1.1.1"  # Include interface name
...
- **+5+**: Major new features, complete module rewrites, breaking changes to that interface

### Files That Display Version:

- `frontend/src/lib/components/desktop-interface/common/Sidebar.svelte` - Desktop version footer and popup content
- `package.json` (root) - Project version in AQ format
- `frontend/package.json` - Frontend project version in AQ format

### Examples:

**Example 1: Fixed bug in Desktop HR module**
```powershell
node scripts/update-version.js desktop    # AQ1.1.1.1 → AQ2.1.1.1
git commit -m "fix(desktop): resolve HR component rendering issue"
```

**Example 2: Added new Mobile app feature**
```powershell
node scripts/update-version.js mobile     # AQ2.1.1.1 → AQ2.2.1.1
git commit -m "feat(mobile): add last punch card display"
```

**Example 3: Major Cashier system overhaul**
```powershell
node scripts/update-version.js cashier 5  # AQ2.2.1.1 → AQ2.2.6.1
git commit -m "refactor(cashier): complete POS system redesign"
```

**Example 4: Updated Customer app for seasonal sale**
```powershell
node scripts/update-version.js customer 2  # AQ2.2.6.1 → AQ2.2.6.3
git commit -m "feat(customer): holiday promotion features"
```

### Commit Message Format:

Always include the interface in parentheses:
```
chore(desktop): bump version to AQ2.1.1.1
chore(mobile): bump version to AQ2.2.1.1
chore(cashier): bump version to AQ2.2.2.1
chore(customer): bump version to AQ2.2.2.2
chore(all): bump version to AQ3.3.3.3
```

### Copilot Workflow Checklist:

Before every `git push`:
- [ ] Run appropriate version update: `node scripts/update-version.js [interface]`
- [ ] Update version popup content in Sidebar.svelte
- [ ] Verify version updated in all 3 locations (root package.json, frontend package.json, Sidebar.svelte)
- [ ] Test version display in desktop interface
- [ ] Verify commit message includes interface name: `chore(desktop): ...`
- [ ] Push to remote repository

### Integration Points:

The version number appears in:
1. **Desktop Sidebar Footer**: Shows version as "AQ1.1.1.1" with clickable popup
2. **Version Popup**: Detailed release notes for current version and interface
...
5. Verify Sidebar.svelte path: `frontend/src/lib/components/desktop-interface/common/Sidebar.svelte`
...
3. `/frontend/src/lib/components/desktop-interface/common/Sidebar.svelte` - Version footer and popup content
...
2. `/frontend/package.json` - "version" field  
3. `/frontend/src/lib/components/desktop-interface/common/Sidebar.svelte` - Version footer and popup content
4. `/frontend/src/routes/mobile-interface/+layout.svelte` - Mobile version badge
...
1. **Use VS Code Task**: Press `Ctrl+Shift+B` or go to Terminal → Run Task
2. **Select Task**: Choose **"🎨 Dev Frontend (npm)"** from the task list
