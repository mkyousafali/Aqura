-- CORRECTED: Remove non-existent category column from create_task function
-- The tasks table doesn't have a category column, so we need to remove it

-- First, create the missing create_task function without category
...
BEGIN
    ...
    IF title_param IS NULL THEN
        RAISE EXCEPTION 'title_param cannot be NULL';
    END IF;
