<script lang="ts">
  ...
  function chooseFiles(taskId: string, event: Event) {
    ...
    filesByTask = { ...filesByTask, [taskId]: [...(filesByTask[taskId] || []), ...newFiles] };
    input.value = '';
  }
              ...
              <td class="table-action">
                ...
                {:else if task.task_number === 1 || task.task_number === 2 || task.task_number === 3}
                  ...
                  {#if task.task_number === 1 || (task.task_number === 3 && balanceByTask[task.id] === 'yes')}
                    <input class="table-file" id={`table-camera-${task.id}`} type="file" accept="image/*" capture="environment" multiple on:change={(event) => chooseFiles(task.id, event)} />
                    <label for={`table-camera-${task.id}`} class="camera-btn table-camera-btn">
          ...
          {:else if task.task_number === 1 || task.task_number === 2 || task.task_number === 3}
            ...
            {#if task.task_number === 1 || (task.task_number === 3 && balanceByTask[task.id] === 'yes')}
              <div class="upload-label">
                ...
                <input class="file-input" id={`camera-${task.id}`} type="file" accept="image/*" capture="environment" multiple on:change={(event) => chooseFiles(task.id, event)} />
                <label for={`camera-${task.id}`} class="camera-btn">
