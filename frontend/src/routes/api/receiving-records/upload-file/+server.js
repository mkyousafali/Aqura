...
export async function POST({ request }) {
	try {
		...
		if (!file_type_str || !['pr_excel', 'original_bill'].includes(file_type_str)) {
			return new Response(
				JSON.stringify({ 
		...
		if (file_type_str === 'pr_excel') {
			const isExcel = file.name.toLowerCase().includes('.xls') || file.name.toLowerCase().includes('.xlsx');
			if (!isExcel) {
				return new Response(
					JSON.stringify({ 
		...
		console.log('📤 [File Upload] Uploading to storage:', { bucketName, filePath });
		...
		const { data: dbData, error: dbError } = await supabase
			...
			.select('id, pr_excel_file_url, original_bill_url, pr_excel_file_uploaded, original_bill_uploaded');
		...
		console.log('✅ [File Upload] Successfully uploaded and updated database:', dbData);
