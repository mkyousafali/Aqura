<script lang="ts">
	...
	onMount(async () => {
		...
		try {
			...
			if (!authenticated && $page.url.pathname !== '/login' && !isCustomerRoute && !isCashierRoute && !isCustomerLoginRoute) {
			...
			const loadingTimeout = setTimeout(() => {
				if (isLoading) {
					...
					if (!isAuthenticated && $page.url.pathname !== '/login' && !isMobileRoute && !isMobileLoginRoute && !isCustomerRouteTimeout && !isCashierRouteTimeout && !isCustomerLoginTimeout) {
			...
			if ($page.url.pathname !== '/login' && !isCustomerRouteError && !isCustomerLoginError) {
