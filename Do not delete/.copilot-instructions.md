...
## 🚀 Go Backend Implementation - Quick Start

**When user says: "Implement Go backend for [Component Name]"**

Follow this automated workflow:

### Step 0: REQUEST TABLE SCHEMA (CRITICAL - MUST DO FIRST)

**⚠️ STOP - Before implementing ANYTHING, ask the user:**

```
I need the exact table schema to implement this correctly. Please:

1. Run: node scripts/create-schema-md.js
2. Open the generated DATABASE_SCHEMA.md file
3. Find the [table_name] section
4. Copy the complete table schema (all columns with types, nullable, defaults)
5. Paste it here
...
Once you provide the schema, I'll implement the complete Go backend with:
- Accurate Go type mappings
- Proper nullable field handling
- Correct column names
- Full CRUD operations with caching
```

**DO NOT PROCEED until user provides the table schema.**

### Step 1: Validate Schema and Identify Component Details
- **Parse provided table schema** - Extract all columns, types, nullable status
- Find the component file path
- Identify the database table name (confirmed from schema)
- Check existing Supabase implementation
- List required operations (GET, CREATE, UPDATE, DELETE)
- Map PostgreSQL types to Go types based on schema

### Step 2: Create Backend Files (After Receiving Schema)

**CRITICAL: Organize by Interface Type**

1. **Determine Interface First:**
   - Check frontend component location:
     - `frontend/src/lib/components/desktop-interface/` → Backend: `desktop-interface/`
     - `frontend/src/lib/components/mobile-interface/` → Backend: `mobile-interface/`
     - `frontend/src/lib/components/customer-interface/` → Backend: `customer-interface/`
     - `frontend/src/lib/components/cashier-interface/` → Backend: `cashier-interface/`

2. **Model**: `backend/models/[interface]/[entity].go`
   - Package name: `package desktopinterface` (or mobileinterface, etc.)
   - **Define struct with EXACT columns from provided schema**
   - **Map PostgreSQL types to Go types:**
     ...
     - `uuid` → `string`
     - `json`/`jsonb` → `json.RawMessage` or custom struct
   - **Use nullable wrappers for nullable columns:**
     - Nullable `text` → `models.NullString`
     - Nullable `bigint` → `sql.NullInt64`
     - Nullable `boolean` → `sql.NullBool`
     - Nullable `numeric` → `sql.NullFloat64`
     - Nullable `timestamp` → `sql.NullTime`
   - Create Input structs for CREATE/UPDATE with pointer fields for optionals
   - Import: `"github.com/mkyousafali/Aqura/backend/models"`

3. **Handler**: `backend/handlers/[interface]/[entity].go`
   - Package name: `package desktopinterface` (or mobileinterface, etc.)
   - Import models: `desktopmodels "github.com/mkyousafali/Aqura/backend/models/desktop-interface"`
   - GET all with caching (5min TTL)
   - GET by ID
...
4. **Routes**: Update `backend/main.go`
   - Import: `desktophandlers "github.com/mkyousafali/Aqura/backend/handlers/desktop-interface"`
   - Add routes with CORS
   - **CRITICAL**: Return `w.WriteHeader(http.StatusOK)` for OPTIONS requests
   - **CRITICAL**: enableCORS must set `Access-Control-Allow-Origin: *` for development
   - **CRITICAL**: Include `apikey` in Access-Control-Allow-Headers
   - **CRITICAL**: Set `Access-Control-Allow-Credentials: true`
   - Map HTTP methods to handlers

4. **PostgreSQL LISTEN/NOTIFY** (for automatic cache updates):
   - Create database trigger in Supabase:
     ```sql
     CREATE OR REPLACE FUNCTION notify_[table]_change() 
     RETURNS trigger AS $$
     BEGIN
         PERFORM pg_notify('[table]_changed',
     ...
     FOR EACH ROW EXECUTE FUNCTION notify_[table]_change();
     ```
   - Add channel to `database/supabase.go` listener:
     ```go
     listener.Listen("[table]_changed")
     ```
   - Add handler in `main.go`:
     ```go
     case "[table]_changed":
         cache.Invalidate("[entity]:all")
         log.Printf("🔄 Cache invalidated for [entity]")
     ```

### Step 3: Create Frontend Integration
Update `frontend/src/lib/utils/goAPI.ts`:
- Add new entity section in `goAPI` object
- Implement all CRUD operations
- Add client-side caching
- Add health check before requests
- Add Supabase fallback for all operations
- Add cache invalidation on mutations

### Step 4: Update Component
- Import `goAPI` instead of direct Supabase
- Update data fetching functions
- Add performance logging

### Step 5: Test & Deploy
- Test endpoints with curl
- Verify caching works (X-Cache: HIT/MISS headers)
- Test LISTEN/NOTIFY:
  - Modify data in Supabase SQL Editor
  - Check backend logs for: `📢 Received notification`
  - Verify cache invalidated: `🔄 Cache invalidated for [entity]`
  - Next request should be cache MISS
- Test fallback system (stop Go backend, verify Supabase fallback)
- Commit and push to Railway/Vercel

**Reference Guide:** See `GO_BACKEND_IMPLEMENTATION_GUIDE.md` for complete patterns and examples.

**Established Pattern:** 
- Backend Model: `backend/models/desktop-interface/branch.go`
- Backend Handler: `backend/handlers/desktop-interface/branches.go`
- Frontend API: `goAPI.branches` in `goAPI.ts`
- Common Types: `backend/models/common.go` (NullString)

**Schema Requirement Checklist:**
- [ ] User provided complete table schema from DATABASE_SCHEMA.md
- [ ] Schema includes all columns with data types
- [ ] Schema shows nullable status for each column
- [ ] Schema shows default values
- [ ] Mapped PostgreSQL types to Go types correctly
- [ ] Used nullable wrappers (NullString, NullInt64, etc.) for nullable columns
- [ ] Verified column names match schema exactly (no typos or assumptions)

---

## 🔄 Version Management Protocol (AQ Format: AQ[Desktop].[Mobile].[Cashier].[Customer])
...
2. ✅ Increments the appropriate interface version (AQ16.2.2.2 → AQ17.2.2.2)
3. ✅ Updates `package.json` (root and frontend)
4. ✅ Updates version display in `Sidebar.svelte`
5. ✅ Generates fresh `VersionChangelog.svelte` with:
...
7. ✅ Creates commit with proper message format
8. ✅ Pushes to origin/master
...
## ⚠️ Important: Always Confirm Before Making Changes
...
- ✋ **Database changes** (schema, functions, triggers, RLS policies)
- ✋ **File creation/deletion** (new components, utilities, routes)
- ✋ **Code modifications** (logic changes, refactoring, optimizations)
- ✋ **Configuration updates** (package.json, vite.config, tailwind.config)
- ✋ **Architectural decisions** (new patterns, state management, API structure)
- ✋ **Version updates** (dependencies, framework versions)
- ✋ **Build/deployment changes** (build scripts, deployment config)
...
1. **Files to Update**:
   - ✅ `frontend/src/lib/i18n/locales/en.ts` - English translations
   - ✅ `frontend/src/lib/i18n/locales/ar.ts` - Arabic translations (RTL)

2. **When Translation is Required**:
   - ✅ New sidebar menu items or buttons
   - ✅ New window titles or component labels
   - ✅ User-facing messages (success, error, warnings)
...
4. **Example Translation Addition**:
   ...
   reports: {
     ...
     salesReport: "Sales Report",  // ← Add new translation
   },
   
   // Arabic (ar.ts)
   reports: {
     expenseTracker: "تتبع المصروفات",
     salesReport: "تقرير المبيعات",  // ← Add new translation
   },
   ```
...
1. **After every `replace_string_in_file` operation** - Check for:
   ...
   - ✅ Proper closing of functions, objects, and arrays
   - ✅ No duplicate closing statements (e.g., `}));` appearing twice)
   - ✅ Correct template literal syntax in Svelte components
   - ✅ Proper JSX/Svelte syntax (no unclosed tags)
...
3. **Common Syntax Pitfalls to Avoid**:
   - ❌ Duplicate closing brackets when copy-pasting code
   - ❌ Missing commas in object/array literals
   - ❌ Unclosed template literals or strings
   - ❌ Missing semicolons in statement termination (where required)
   - ❌ Incorrect arrow function syntax `() => {}`
   - ❌ Mismatched JSX/Svelte tag closures
...
1. **Before Adding ANY Import Statement**:
   - ✅ Use `file_search` to find the actual file/module location
   - ✅ Check existing imports in similar files
   - ✅ Use `grep_search` to find how the module is imported elsewhere
   - ✅ Verify the export name matches what you're importing
...
3. **Import Path Patterns in Aqura**:
   - ✅ `$lib/stores/notifications` - Notification store (uses .add() method)
   - ✅ `$lib/stores/windowManager` - Window management store
   - ✅ `$lib/i18n` - Bilingual translation system
   - ✅ `$lib/utils/supabase` - Supabase client utilities
   - ✅ `$lib/components/*` - UI components
   - ❌ `$lib/stores/notificationStore` - DOES NOT EXIST
   - ❌ `$lib/utils/notifications` - WRONG PATH

4. **Common Import Mistakes to Avoid**:
   - ❌ Importing non-existent modules (e.g., `notificationStore`)
   - ❌ Using wrong import names (e.g., `showNotification` instead of `notifications`)
   - ❌ Mixing default and named imports incorrectly
   - ❌ Using relative paths instead of `$lib` alias
   - ❌ Assuming a utility exists without checking
   - ❌ Copy-pasting imports from other projects
...
1. **Before Calling ANY Method**:
   - ✅ Read the source file to see available methods
   - ✅ Check method signatures (parameters, return types)
   - ✅ Look for existing usage examples in codebase
   - ✅ Verify method name spelling exactly
...
3. **Method Call Patterns in Aqura**:
   - ✅ `notifications.add({ message, type, duration })` - Add notification
   - ✅ `openWindow('Component', { props })` - Open window
   - ✅ `$t('translation.key')` - Get translation
   - ❌ `showNotification({ message })` - DOES NOT EXIST
   - ❌ `notifications.show()` - WRONG METHOD NAME
...
3. **When Loading Data from Database**:
   ```javascript
   // ✅ CORRECT - Convert UTC to Saudi time for display
   offerData = {
...
4. **When Saving Data to Database**:
   ```javascript
   // ✅ CORRECT - Convert Saudi time to UTC for storage
   const offerPayload = {
...
5. **Common Timezone Mistakes to Avoid**:
   - ❌ Saving datetime-local value directly without UTC conversion
   - ❌ Displaying UTC time directly in datetime-local input
   - ❌ Forgetting that datetime-local is in local timezone (Saudi time)
   - ❌ Assuming server and client are in same timezone
   - ❌ Using `new Date()` without timezone consideration

6. **When to Use Timezone Conversion**:
   - ✅ **ALWAYS** for datetime-local inputs in forms
   - ✅ **ALWAYS** when saving/loading start_date, end_date, scheduled times
   - ✅ **ALWAYS** for offer validity periods
   - ✅ **ALWAYS** for task due dates
   - ✅ **ALWAYS** for scheduled notifications
...
8. **Impact of Incorrect Timezone Handling**:
   - ❌ Offers scheduled for wrong times (3 hours off)
   - ❌ Offers showing as "scheduled" when they should be "active"
   - ❌ Offers disappearing from lists due to status calculation errors
   - ❌ Tasks due at wrong times
...
```typescript
// ❌ WRONG - Direct mutation - Svelte won't detect changes!
export const updateWindowPosition = (windowId: string, position: { x: number; y: number }) => {
  windows.update(store => {
    ...
    window.position = position;  // ← MUTATION - Store doesn't see this change
    return store;  // ← Same object reference returned
  });
};
...
export const updateWindowPosition = (windowId: string, position: { x: number; y: number }) => {
  windows.update(store => {
    const newWindows = new Map(store);  // ← Create new Map
    const window = newWindows.get(windowId);
    newWindows.set(windowId, { ...window, position });  // ← Spread creates new object
    return newWindows;  // ← Return new reference, Svelte detects this!
  });
};
```
...
```typescript
// ❌ WRONG - What was broken before fix
...
// ✅ CORRECT - Creates new nested objects
export const updateConfig = (config: Config) => {
  appConfig.update(value => ({
    ...value,
    settings: {
...
- ❌ `store.property = value` - Direct assignment, returns same reference
- ❌ `array.push(item)` - Mutates array, same reference
- ❌ `map.set(key, value)` - If not wrapping in new Map(), doesn't trigger reactivity
- ❌ `object.nested.property = value` - Deep mutation, same root reference
- ❌ Reassigning array element: `array[0] = newValue` - Same reference

**Common Correct Patterns:**
- ✅ `new Map(oldMap)` - Creates new Map reference
- ✅ `{ ...object, property: value }` - Creates new object reference
- ✅ `[...array]` or `array.filter()` - Creates new array reference
- ✅ Spread operator for nested: `{ ...obj, nested: { ...obj.nested, prop: val } }`
- ✅ Array methods that return new arrays: `map()`, `filter()`, `concat()`, `slice()`
...
const activateWindow = (windowId: string) => {
  windows.update(store => {
    const newWindows = new Map(store);  // ← NEW MAP
    const window = newWindows.get(windowId);
    newWindows.set(windowId, { ...window, state: 'normal' });  // ← NEW OBJECT
    return newWindows;  // ← NEW REFERENCE (Svelte sees this!)
  });
};
```
...
2. **Reactive Statement Test**: If a reactive statement doesn't trigger, mutation is likely
   ...
   <script>
     ...
     $: myValue = $store.someProperty;  // ← Should update when store changes
   </script>
   ```
...
2. **When Adding New UI Labels**:
   - ✅ Add translation key to English file (en.ts)
   - ✅ Add translation key to Arabic file (ar.ts)
   - ✅ Use IDENTICAL key names in both files
   - ✅ Place translations in the appropriate section (hr, admin, reports, etc.)
   - ❌ Don't add to just one language file
   - ❌ Don't use different key names between languages
   - ❌ Don't create duplicate keys (will cause compilation errors)

3. **Translation Key Naming Conventions**:
   ```typescript
   // ✅ CORRECT - Organized by feature section
   hr: {
   ...
   // ❌ WRONG - Flat structure without sections
   presentToday: "Present Today",
   branchBreakdown: "Branch-wise Breakdown",
   ```
...
8. **Common Language Support Errors to Avoid**:
   - ❌ `store_invalid_shape` - Using `$t()` instead of `t()` (not a store!)
   - ❌ `Cannot read properties of undefined` - Translation key doesn't exist
   - ❌ Duplicate key errors - Same key defined twice in object
   - ❌ Missing Arabic translation - Key only in en.ts, not ar.ts
   - ❌ Missing English translation - Key only in ar.ts, not en.ts
   - ❌ Hardcoded English text - Should use `t('key')` instead
   - ❌ Wrong import path - `$lib/i18n/locales` instead of `$lib/i18n`

9. **Translation File Error Prevention**:
   - ✅ Always check `get_errors` after modifying en.ts or ar.ts
   - ✅ Use VS Code's bracket matching to verify object structure
   - ✅ Count opening/closing braces to ensure balance
   - ✅ Look for duplicate comma errors or missing commas
   - ✅ Verify no trailing commas before closing braces
   - ✅ Ensure each section has matching structure in both files
...
```typescript
// ❌ WRONG - w-screen includes scrollbar width, causes horizontal overflow
...
**The Solution (CORRECT PATTERN):**
```typescript
// ✅ CORRECT - Proper full-page container with flex constraints
...
```svelte
<!-- ✅ CORRECT - Full-page layout with 2-column grid -->
<div class="flex flex-col w-full h-screen bg-gradient-to-br from-gray-50 to-gray-100 overflow-hidden">
...
- ✅ **Searching codebase** to understand structure
- ✅ **Checking database schema** using utility scripts
- ✅ **Providing information** or explanations
- ✅ **Suggesting solutions** without implementing them
...
- ✅ **Keep it brief and conversational** - Use simple, friendly language
- ✅ **Focus on what was accomplished** - Describe the outcome, not the technical details
- ✅ **Use bullet points** - Easy to scan and understand
- ✅ **Avoid code snippets** - Unless specifically requested by the user
- ✅ **Be human and relatable** - "Fixed the issue where offers were disappearing" instead of "Modified RLS policy execution"
- ✅ **Highlight user benefits** - "Now you can see and manage paused offers" instead of "Implemented reactive filtering"

### Never Assume:
- ❌ Don't assume column names exist in database tables - CHECK FIRST
- ❌ Don't assume functions or triggers exist - VERIFY using utility scripts
- ❌ Don't assume file structure - LIST directories if unsure
- ❌ Don't assume dependencies are installed - CHECK package.json
- ❌ Don't assume user wants a specific implementation - ASK for preferences
- ❌ Don't assume backwards compatibility is not needed - CONFIRM breaking changes
...
- **Go Backend**: Railway deployment with 5-min in-memory caching
- **API Layer**: Go backend with automatic Supabase fallback
- **Deployment**: Vercel (Frontend) + Railway (Go Backend) + Supabase (Database)
- **Build Tools**: pnpm monorepo + Vite + PWA + Electron packaging
- **Package Manager**: pnpm (required - NOT npm/yarn)

### Development Environment:

**IMPORTANT: Run both frontend and backend servers for full-stack development.**

#### Option 1: VS Code Task (Recommended - Both Servers)
1. **Press `Ctrl+Shift+P`**
2. **Type**: "Run Task"
3. **Select**: "🔥 Start Both Servers (Frontend + Go)"
...
2. Select **"🎨 Dev Frontend (npm)"**
3. Server starts on http://localhost:5173

**Backend Only:**
1. Terminal → Run Task
2. Select **"🚀 Go Backend Server"**
...
- **🎨 Dev Frontend (npm)** - START LOCAL DEV SERVER (RECOMMENDED)
- 🚀 Build All - Build all packages
- 📱 Build PWA - Create PWA build
- 🪟 Build Windows Installer - Create Windows installer
- 🧹 Lint All - Run linters
- 🧪 Test All - Run tests

### Key Features Implemented:
- ✅ **Windowed Desktop Interface**: Drag/resize/minimize windows, taskbar, command palette
- ✅ **PWA Capabilities**: Installable, offline-first, service worker, push notifications
- ✅ **Bilingual System**: Arabic/English with RTL support and cultural adaptations
- ✅ **Business Modules**: 
...
- ✅ **Accessibility**: WCAG 2.1 compliant, screen reader support, keyboard navigation
- ✅ **Security**: Supabase RLS, JWT auth, encrypted transmission
...
- **Usage Examples**:
  - `offers.created_by` → References admin user who created the offer
  - `tasks.created_by` → References admin user who created the task
  - `notifications.created_by` → References admin user who sent the notification
...
- **Usage Examples**:
  - `hr_employees.id` → Employee records for HR purposes
  - `hr_position_assignments.employee_id` → Position tracking
  - `hr_salary_components.employee_id` → Salary management
  - `expense_requisitions.employee_id` → Employee expense requests
...
- **Usage Examples**:
  - `offer_usage_logs.customer_id` → Customer who used an offer
  - `orders.customer_id` → Customer who placed the order (when orders table exists)
  - `customer_recovery_requests.customer_id` → Customer account recovery
...
| **Can Create Offers?** | ✅ Yes | ❌ No | ❌ No |
| **Can Receive Offers?** | ❌ No | ❌ No | ✅ Yes |
| **Can Place Orders?** | ❌ No | ❌ No | ✅ Yes |
| **In HR System?** | Maybe (if also employee) | ✅ Yes | ❌ No |

#### Common Mistakes to Avoid:
- ❌ **Don't confuse** `users` with `employees` - they serve different purposes
- ❌ **Don't assume** all employees have user accounts (most don't)
- ❌ **Don't use** `hr_employees` table for offer assignments (use `customers` table)
- ❌ **Don't reference** `employees` in customer-facing features (use `customers` table)
- ❌ **Don't mix** admin operations (`users`) with customer operations (`customers`)

#### When Creating New Features:
- **Admin/Manager Actions** → Use `users` table (created_by, assigned_by, managed_by)
- **Customer-Facing Features** → Use `customers` table (customer_id, user_id in customer context)
- **HR Operations** → Use `hr_employees` table (employee_id for payroll, attendance, positions)
...
**IMPORTANT NOTES:**
- ❌ `customer_offers` table has been REMOVED (not needed - offers are global)
- ✅ All redundant columns removed from `offers` table (2025-11-14 cleanup)
- ✅ Offer-specific data properly normalized into child tables
- ✅ Always query child tables for offer details (products, bundles, BOGO, tiers)
...
- `hr_employees.branch_id` → `branches.id`
- `tasks.assigned_to` → `users.id`
- `receiving_records.vendor_id` → `vendors.id`
- `notifications.created_by` → `users.id`
- `expense_requisitions.employee_id` → `hr_employees.id`
...
│   ├── src/
│   │   ├── lib/
│   │   │   ├── components/    # Reusable UI components (INTERFACE-BASED ORGANIZATION)
│   │   │   │   ├── common/            # Shared components across all interfaces
│   │   │   │   │   ├── FileUpload.svelte
│   │   │   │   │   ├── FileDownload.svelte
│   │   │   │   │   ├── NotificationSoundControls.svelte
│   │   │   │   │   ├── ToastNotifications.svelte
│   │   │   │   │   ├── UserSwitcher.svelte
│   │   │   │   │   ├── WelcomeWindow.svelte
│   │   │   │   │   ├── Window.svelte              # Core window component
│   │   │   │   │   └── WindowManager.svelte       # Window management system
│   │   │   │   │
│   │   │   │   ├── desktop-interface/  # Desktop admin interface (36+ components)
│   │   │   │   │   ├── common/         # Sidebar, Taskbar, CommandPalette
│   │   │   │   │   ├── master/         # Master data (Branch, HR, Task, Vendor, etc.)
│   │   │   │   │   ├── work/           # Work management (receiving, tasks, requisitions)
│   │   │   │   │   ├── admin-customer-app/  # Customer app management
│   │   │   │   │   │   └── products/   # Product management (8 components)
│   │   │   │   │   ├── communication/  # Notifications, messages
│   │   │   │   │   └── settings/       # Settings (7 components + user management)
│   │   │   │   │
│   │   │   │   ├── customer-interface/  # Customer shopping interface (7 components)
│   │   │   │   │   ├── common/         # TopBar, BottomNav, CustomerLogin
│   │   │   │   │   ├── cart/           # BottomCartBar
│   │   │   │   │   └── shopping/       # FeaturedOffers, OfferBadge, OfferDetailModal
│   │   │   │   │
│   │   │   │   ├── mobile-interface/   # Mobile employee interface (4 components)
│   │   │   │   │   ├── common/         # LanguageToggle
│   │   │   │   │   ├── notifications/  # NotificationCenter
│   │   │   │   │   └── tasks/          # TasksWindow, QuickTaskModal, TaskCompletionModal
│   │   │   │   │
│   │   │   │   └── cashier-interface/  # Cashier/POS interface (4 components)
│   │   │   │       ├── CashierInterface.svelte   # Standalone desktop interface
│   │   │   │       ├── CashierLogin.svelte       # Access code authentication
│   │   │   │       ├── CashierTaskbar.svelte     # Taskbar for cashier
│   │   │   │       └── CouponRedemption.svelte   # Coupon redemption window
│   │   │   │
...
│   │   │   ├── +layout.server.ts     # Server config
│   │   │   ├── +page.svelte          # Root router/redirect
│   │   │   ├── login/                # Main interface selector ✅
│   │   │   │   └── +page.svelte
│   │   │   │
│   │   │   ├── desktop-interface/    # Desktop admin routes ✅
│   │   │   │   ├── +layout.svelte
│   │   │   │   ├── +page.svelte
│   │   │   │   └── receiving/        # Receiving workflow pages
│   │   │   │
│   │   │   ├── customer-interface/   # Customer shopping routes ✅
│   │   │   │   ├── +layout.svelte
│   │   │   │   ├── +page.svelte
...
│   │   │   │   └── assignments/
│   │   │   │
│   │   │   ├── cashier-interface/    # Cashier routes ✅
│   │   │   │   └── +page.svelte
│   │   │   │
...
│   ├── migrations/            # Database migrations (SQL files removed)
...
- ✅ **common/** - Shared across ALL interfaces (window system, file upload, notifications)
- ✅ **desktop-interface/** - Desktop admin components ONLY
- ✅ **customer-interface/** - Customer shopping app components ONLY
- ✅ **mobile-interface/** - Mobile employee app components ONLY
- ✅ **cashier-interface/** - Cashier/POS components ONLY
- ❌ **Never mix** components from different interfaces
- ❌ **Never use** `admin/`, `mobile/`, `customer/` - use proper interface names

### Routes Organization (✅ COMPLETED - See ROUTES_REORGANIZATION_PLAN.md):
- ✅ **Completed:** Routes renamed to match component structure
  - `/customer` → `/customer-interface` ✅
  - `/mobile` → `/mobile-interface` ✅
  - `/cashier` → `/cashier-interface` ✅
  - Login pages moved:
    - `/login` → Root level (main interface selector) ✅
    - `/mobile-login` → `/mobile-interface/login` ✅
    - `/customer-login` → `/customer-interface/login` ✅
    - `/register` → `/customer-interface/register` ✅
...
## 🔍 Direct Data Query Protocol - ALWAYS Use Service Role Key

**CRITICAL: Whenever you need to check or inspect ACTUAL DATA from any table, ALWAYS query directly using the service role key. NEVER assume, guess, or use sample data.**

### When to Use Direct Data Queries:

- ✅ Checking what data actually exists in a table
- ✅ Verifying recent data changes or updates
- ✅ Inspecting specific records before making changes
- ✅ Validating data integrity or field values
- ✅ Debugging data-related issues
- ✅ Understanding actual user/customer/employee records
- ✅ Checking branch-specific data filtering
- ✅ Verifying timestamps, statuses, or relationships
- ✅ Any situation where you say "let me check the data"

### Quick Query Script Template:

**Location:** `scripts/fetch-[table-name].js`
...
const supabaseUrl = envVars.VITE_SUPABASE_URL;
const supabaseServiceKey = envVars.VITE_SUPABASE_SERVICE_KEY;  // ← CRITICAL: Use SERVICE_KEY not ANON_KEY

console.log('🔍 Fetching data from [table_name]...\n');

// Fetch data - adjust filters/limit as needed
const response = await fetch(`${supabaseUrl}/rest/v1/[table_name]?limit=5`, {
  method: 'GET',
  headers: {
...
### Key Details for Service Role Key:

- **Environment Variable:** `VITE_SUPABASE_SERVICE_KEY` (in `frontend/.env`)
- **NOT:** `VITE_SUPABASE_ANON_KEY` - that's for frontend only
- **NOT:** `VITE_SUPABASE_SERVICE_ROLE_KEY` - that's old naming, use `VITE_SUPABASE_SERVICE_KEY`
- **Permissions:** Service key has full database access without RLS restrictions
- **Usage:** Backend operations, direct data inspection, admin queries
...
curl -H "Authorization: Bearer ${SERVICE_KEY}" \
  "${SUPABASE_URL}/rest/v1/users?status=eq.active&limit=10"
```
...
const response = await fetch(
  `${supabaseUrl}/rest/v1/users?id=eq.user-id-here`,
  { headers }
);

// Filter by multiple conditions
const response = await fetch(
  `${supabaseUrl}/rest/v1/tasks?assigned_to=eq.user-id&status=eq.pending`,
  { headers }
);
```

**Check relationships and joins:**
```javascript
// Include related data
const response = await fetch(
  `${supabaseUrl}/rest/v1/tasks?select=*,assigned_user:users(username,email)`,
  { headers }
);
```

### Workflow for Data Inspection:

1. **Identify what you need to check**
   - "I need to see what users are in the system"
   - "I need to verify the branch_id for this user"
   - "I need to check if tasks are being created correctly"

2. **Create a quick script** (use template above)
   - Location: `scripts/fetch-[description].js`
   - Replace `[table_name]` with actual table
   - Adjust limit, filters as needed

3. **Run the script**
   ```bash
   node scripts/fetch-[table-name].js
   ```

4. **Analyze the output**
   - Check field names and data types
   - Verify expected relationships
   - Look for null values or unexpected data
   - Check timestamps and statuses

5. **Share findings with user** (no code unless requested)
   - "I found 5 active users in branch 3"
   - "The task status is 'pending', not 'completed'"
   - "The user_id field is null in this record"

### Common Queries in Development:

```bash
# Check users (first 5)
node scripts/fetch-users.js

# Check tasks for a branch
node scripts/fetch-tasks.js

# Check customers
node scripts/fetch-customers.js

# Check offers
node scripts/fetch-offers.js

# Check receiving records
node scripts/fetch-receiving.js
```

### Error Handling:

**If query fails with 401 Unauthorized:**
- ✅ Verify using `VITE_SUPABASE_SERVICE_KEY` (not `SERVICE_ROLE_KEY`)
- ✅ Check `.env` file has credentials
- ✅ Verify credentials are valid (not expired)

**If query returns empty array:**
- ✅ Table might be empty - check if it should have data
- ✅ Filters might be too restrictive - try removing filters
- ✅ RLS policies might be blocking (use service key to bypass)

**If you get schema error:**
- ✅ Table name might be wrong - check DATABASE_SCHEMA.md
- ✅ Column name might be wrong - verify exact spelling
- ✅ Run `node scripts/create-schema-md.js` to refresh schema

### Verification Checklist:

- [ ] Using `VITE_SUPABASE_SERVICE_KEY` from `.env`
- [ ] Script reads from correct `.env` file
- [ ] Query URL has correct table name
- [ ] Checked actual data before making assumptions
- [ ] Results match expected schema structure
- [ ] No hardcoded values - always use actual table data

---

## 📊 Fetching Database Schema, Functions, and Storage from Supabase
...
5. **Verify Your Assumptions:**
   - Does the table exist? ✅ Check `DATABASE_SCHEMA.md`
   - Does the column exist? ✅ Check `DATABASE_SCHEMA.md`
   - Does the function exist? ✅ Check `DATABASE_FUNCTIONS.md`
   - Can I upload to this bucket? ✅ Check `STORAGE_INFO.md`
   - What are the file type restrictions? ✅ Check `STORAGE_INFO.md`

6. **THEN proceed** with development

### Critical Rules - NEVER Do These:

❌ **NEVER assume a table exists** without checking `DATABASE_SCHEMA.md`
❌ **NEVER assume a column exists** without checking `DATABASE_SCHEMA.md`
❌ **NEVER call a function** without checking `DATABASE_FUNCTIONS.md`
❌ **NEVER upload files** without checking `STORAGE_INFO.md` for bucket and MIME type
❌ **NEVER hardcode bucket names** - always use the documented names
❌ **NEVER use outdated documentation** - always fetch fresh
❌ **NEVER assume file size limits** - check `STORAGE_INFO.md` for current limits
❌ **NEVER assume MIME type restrictions** - verify in `STORAGE_INFO.md`
