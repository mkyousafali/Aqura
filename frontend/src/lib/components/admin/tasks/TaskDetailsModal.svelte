...
<div class="task-details-modal p-6 space-y-6 bg-white">
	...
	{:else}
		...
		{#if taskImages && taskImages.length > 0}
			<div class="space-y-4">
				<h3 class="text-lg font-semibold text-gray-900 border-b pb-2">Task Images ({taskImages.length})</h3>
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					{#each taskImages as image, index}
						<div class="border border-gray-200 rounded-lg overflow-hidden bg-white shadow-sm hover:shadow-md transition-shadow">
							<div class="relative aspect-video bg-gray-50">
								<img 
									...
									on:click={() => window.open(image.file_url, '_blank')}
									on:error={(e) => {
								...
								/>
								<div class="absolute inset-0 bg-red-100 border-2 border-red-300 border-dashed rounded flex items-center justify-center text-red-600 text-sm" style="display: none;">
		...
		{:else}
			<div class="space-y-4">
				<h3 class="text-lg font-semibold text-gray-900 border-b pb-2">Task Images</h3>
				<div class="text-center py-8 text-gray-500">
					<svg class="w-12 h-12 mx-auto text-gray-300 mb-2" fill="currentColor" viewBox="0 0 20 20">
						<path fill-rule="evenodd" d="M4 3a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V5a2 2 0 00-2-2H4zm12 12H4l4-8 3 6 2-4 3 6z" clip-rule="evenodd"/>
					</svg>
					<p>No images attached to this task</p>
				</div>
			</div>
		{/if}
