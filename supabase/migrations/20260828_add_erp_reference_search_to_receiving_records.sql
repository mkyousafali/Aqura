DROP FUNCTION IF EXISTS public.get_receiving_records_with_details(integer, integer, text, text, text, text);
DROP FUNCTION IF EXISTS public.get_receiving_records_with_details(integer, integer, text, text, text, text, text);
...
CREATE FUNCTION public.get_receiving_records_with_details(
    ...
    pr_excel_verified_date timestamptz, total_count bigint
...
BEGIN
    ...
    SELECT r.id::text, r.bill_number::text, r.vendor_id::text, r.branch_id::text,
        ...
        ps.pr_excel_verified_date, v_total
    FROM receiving_records r
