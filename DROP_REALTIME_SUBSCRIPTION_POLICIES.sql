-- ============================================================================
-- DROP REALTIME_SUBSCRIPTION RLS POLICIES FROM ALL 88 TABLES
-- ============================================================================
-- Date: December 8, 2025
-- Purpose: Remove the "realtime_subscription" RLS policy from all tables
-- Note: This will disable real-time subscriptions for authenticated users
-- Execution Time: ~2-5 minutes (depending on database size)
-- ============================================================================

-- ============================================================================
-- PART 1: DROP REALTIME_SUBSCRIPTION POLICIES (88 tables)
-- ============================================================================
...
-- Verify all realtime_subscription policies have been dropped
SELECT 
  schemaname,
  tablename,
  policyname,
...
-- Count remaining realtime_subscription policies (should be 0)
SELECT COUNT(*) as total_realtime_policies 
FROM pg_policies 
WHERE policyname = 'realtime_subscription';
...
--
-- WHAT WAS REMOVED:
-- ✅ Dropped "realtime_subscription" policy from 88 tables
-- ✅ This disables real-time subscriptions for authenticated users
--
-- IMPACT:
-- - Real-time subscriptions will no longer work
-- - WebSocket connections will fail with CHANNEL_ERROR
-- - Data can still be queried normally (other policies remain)
-- - Other RLS policies (anon_full_access, authenticated_full_access) are unaffected
--
-- HOW TO VERIFY EXECUTION:
-- 1. Run the verification queries above
-- 2. Should see 0 rows in both result sets
-- 3. Check browser console - you should see CHANNEL_ERROR messages
-- 4. Data loading will work, but real-time updates will not show
--
-- TO RESTORE REALTIME:
-- Re-run the REALTIME_RLS_POLICY.sql script to recreate all policies
--
-- ============================================================================
