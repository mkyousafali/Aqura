<script>
  ...
  async function calculateDrivingDistances() {
    // AQ-SEC-005: the Google API key is no longer fetched into the browser — this now
    // calls the server proxy, which looks up the key itself. Haversine fallback below
    // still covers any branch that doesn't get a Routes API result (key missing, etc).
