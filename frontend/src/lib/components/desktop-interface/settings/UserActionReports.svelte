<script lang="ts">
	...
	// into one bucket was hiding everything else). Anything not listed here falls back to the raw code
	// via deriveSaveKind rather than silently disappearing into a generic bucket.
	const SAVE_FORM_NAMES: Record<string, string> = {
		SIVAT: 'Sales Invoice', SI: 'Sales Invoice',
		SRVAT: 'Sales Return', SR: 'Sales Return',
		PI: 'Purchase Invoice', PIVAT: 'Purchase Invoice (VAT)',
		PR: 'Purchase Return', PRVAT: 'Purchase Return (VAT)',
		PO: 'Purchase Order', SO: 'Sales Order',
		GRN: 'Goods Received Note', GRR: 'Goods Return',
		CP: 'Cash Payment', CR: 'Cash Receipt',
		BP: 'Bank Payment', BR: 'Bank Receipt',
		JV: 'Journal Voucher', MJV: 'Manual Journal Voucher',
		CN: 'Credit Note', DN: 'Debit Note',
		EX: 'Expense', DMG: 'Damage / Write-off',
		SA: 'Stock Adjustment', OB: 'Opening Balance',
		BTO: 'Branch Transfer Out', MNFPRD: 'Manufacturing',
		Settings: 'App Settings', USERS: 'User Management',
		USERTYP: 'User Types', BRANCH: 'Branch Management',
	...
	// Fixed display order for the "All Actions Breakdown" pills, grouped by workflow rather than
	// sorted by count — makes the pill row scannable/predictable instead of reshuffling every time
	// the date range or cashier filter changes the counts. Any kind not listed here (a new Save form
	// code, an admin/settings action, etc.) still shows — it's just appended after these, sorted by
	// count like before, so nothing silently disappears.
	const KIND_ORDER = [
		// Row 1 — Sales / Item Actions
		'Sales Invoice', 'Price Change', 'Void selected item', 'Cancel selected product list', 'Remove a row from list',
		// Row 2 — Sales / Return / Counter
		'Sales Return', 'Goods Return', 'Counter Open/Close', 'Open return window', 'Save (SUB)',
		// Row 3 — Purchase
		'Purchase Order', 'Purchase Invoice', 'Purchase Invoice (VAT)', 'Goods Received Note', 'Purchase Return (VAT)',
		// Row 4 — Cash / Bank
		'Cash Receipt', 'Cash Payment', 'Bank Receipt', 'Bank Payment', 'Journal Voucher',
		// Row 5 — General Actions
		'View', 'Edit', 'Print', 'Open', 'Try to close window',
		// Row 6 — Other / Accounting / Transfer
		'Manual Journal Voucher', 'Other Authorize', 'Branch Transfer Out'
	];
	const KIND_ORDER_INDEX = new Map(KIND_ORDER.map((kind, i) => [kind, i]));

	// The 3 kinds that stay red in the breakdown pills (money/stock actually leaving) — everything
	// else is green.
	const RISK_KINDS = new Set(['Void selected item', 'Cancel selected product list', 'Sales Return']);

	// Display-only mapping from the canonical (always-English) `kind` value used for filtering/keying
	// above to a translation key under userActionReports.kinds — keeps every Map/Set key, sort order,
	// and CSS class check untouched (they compare against the raw kind), while the table/pills only
	// ever show the localized label via kindLabel() below.
	const KIND_LABEL_KEY: Record<string, string> = {
		'Sales Invoice': 'salesInvoice',
		'Sales Return': 'salesReturn',
		'Purchase Invoice': 'purchaseInvoice',
		'Purchase Invoice (VAT)': 'purchaseInvoiceVat',
		'Purchase Return': 'purchaseReturn',
		'Purchase Return (VAT)': 'purchaseReturnVat',
		...
		'Manual Journal Voucher': 'manualJournalVoucher',
		'Credit Note': 'creditNote',
		'Debit Note': 'debitNote',
		'Expense': 'expense',
		'Damage / Write-off': 'damageWriteOff',
		'Stock Adjustment': 'stockAdjustment',
		'Opening Balance': 'openingBalance',
		'Branch Transfer Out': 'branchTransferOut',
		'Manufacturing': 'manufacturing',
		'App Settings': 'appSettings',
		'User Management': 'userManagement',
		'User Types': 'userTypes',
		'Branch Management': 'branchManagement',
		'Counter Management': 'counterManagement',
		'Save (Other)': 'saveOther',
		'Price Change': 'priceChange',
		'BTI Pending': 'btiPending',
		'BTI Accepted': 'btiAccepted',
		'Cheque Print': 'chequePrint',
		'Product Transaction': 'productTransaction',
		'Counter Open/Close': 'counterOpenClose',
		'Remove a row from list': 'removeRowFromList',
		'Void selected item': 'voidSelectedItem',
		'Cancel selected product list': 'cancelSelectedProductList',
		'Open return window': 'openReturnWindow',
		'Try to close counter shift': 'tryCloseCounterShift',
		'Try to close window': 'tryCloseWindow',
		'Other Authorize': 'otherAuthorize',
		'View': 'view',
		'Edit': 'edit',
		'Print': 'print',
		'Open': 'open',
	...
	// Any Save form code not in SAVE_FORM_NAMES falls back to the raw "Save (CODE)" string as its
	// canonical kind — the code itself (e.g. an ERP voucher-type abbreviation) isn't translatable,
	// but the surrounding "Save (...)" wording still is.
	function kindLabel(kind: string): string {
		const key = KIND_LABEL_KEY[kind];
		if (key) return $t(`userActionReports.kinds.${key}`);
		const saveMatch = /^Save \((.+)\)$/.exec(kind);
		if (saveMatch) return `${$t('userActionReports.kinds.saveGeneric')} (${saveMatch[1]})`;
		return kind;
	}
	...
	// Categorize a Save action by which ERP form/voucher type it saved (SIVAT = sales invoice,
	// PIVAT = purchase invoice, etc.) instead of lumping every save into one undifferentiated bucket.
	function deriveSaveKind(actionForm: string | null | undefined): string {
		const form = (actionForm || '').trim();
		if (!form) return 'Save (Other)';
		return SAVE_FORM_NAMES[form] || `Save (${form})`;
	}

	// Categorize any UserActions row (Authorize/Save or otherwise) into a display Kind
	function deriveKind(actionName: string, actionPerformed: string | null | undefined, actionForm: string | null | undefined): string {
		...
		if (actionName === 'Save') return deriveSaveKind(actionForm);
	...
	// Groups an item-level row into the "action instance" it belongs to -- same key as groupByEvent
	// below. Item-level kinds (Void selected item, Cancel selected product list, Remove a row from
	// list) log one row PER PRODUCT touched, so a single cancel/void of a 5-item basket produces 5
	// rows. Counting raw rows for those kinds reports the product count, not how many times the
	// action was actually performed -- eventKeyOf() below collapses same counter+second+kind+
	// authorizer rows back into one action instance for counting purposes.
	function eventKeyOf(row: Entry) {
		const secondBucket = row.time.slice(0, 19);
		return `${row.counter}|${secondBucket}|${row.kind}|${row.authorizedBy || ''}`;
	}
	...
	$: summaryRows = Array.from(
		cashierScopedEntries.reduce((map, r) => {
			// Item-level kinds count distinct action instances (entries, via eventKeyOf() above), not
			// the individual product rows logged per instance; every other kind is already one row
			// per instance, so raw row count is correct for those -- give each of those rows its own
			// unique key (the bucket's current size) so none collide in the Set.
			const bucket = map.get(r.kind) || new Set<string>();
			bucket.add(ITEM_LEVEL_KINDS.has(r.kind) ? eventKeyOf(r) : String(bucket.size));
			map.set(r.kind, bucket);
			return map;
		}, new Map<string, Set<string>>()).entries()
	).map(([Kind, keys]) => ({ Kind, Cnt: keys.size })).sort((a, b) => {
		const ai = KIND_ORDER_INDEX.has(a.Kind) ? KIND_ORDER_INDEX.get(a.Kind)! : Infinity;
		const bi = KIND_ORDER_INDEX.has(b.Kind) ? KIND_ORDER_INDEX.get(b.Kind)! : Infinity;
		if (ai !== bi) return ai - bi;
		return b.Cnt - a.Cnt; // unlisted kinds fall back to count-desc, same as before
	});
	...
	// Per-actor breakdown (respects the active Kind/Cashier filters), sorted by total amount desc,
	// falls back to count. Built from groupedRows (not filteredEntries directly) so "bills" counts
	// action instances -- for item-level kinds a single bill can span several product rows, and
	// counting those raw rows would report products handled instead of bills processed.
	$: cashierBreakdown = Array.from(
		groupedRows.reduce((map, g) => {
			const key = g.actor || 'Unknown';
			const entry = map.get(key) || { name: key, bills: 0, total: 0 };
			entry.bills += 1;
			entry.total += g.total;
			map.set(key, entry);
			return map;
		}, new Map<string, { name: string; bills: number; total: number }>()).values()
	).sort((a, b) => (b.total - a.total) || (b.bills - a.bills));
	...
	async function loadBranches() {
		...
		try {
			...
			errorMessage = err.message || $t('userActionReports.failedToLoadBranches');
		} finally {
	...
	async function loadReport() {
		...
		try {
			...
			//
			// For 'Authorize' rows, UserActions.UserID is the APPROVER (whoever entered the PIN), not the
			// cashier working the till — there's no VoidItems-style sibling row logging a separate requester.
			// CounterShift does track that: it has one open/close row per counter per till-session with its
			// own OpenUserID. We correlate the Authorize event's CounterID + timestamp against whichever
			// CounterShift row was open at that moment (OpenTime/ClosingTime are TIME-only, so the date comes
			// from TransactionDate — handled here by re-stitching them, including the case where a shift spans
			// past midnight into the next calendar day) and surface that shift's OpenUserID as the requesting
			// user. The "ua.ActionName = 'Authorize'" guard inside the APPLY short-circuits it for every other
			// action kind so the join cost is only paid on Authorize rows.
			// Some Save flows are gated by a manager Authorize action just beforehand — confirmed live:
			// every "Sales Return" (SRVAT) save is preceded, on the same counter, by an "Open return
			// window" Authorize ~10-15s earlier. There's no explicit link between the two rows in the
			// schema (unlike VoidItems, which has its own CounterShiftID/UserID), so matching has to be
			// by counter+time proximity — same trade-off the VoidItems ±5s match above already makes.
			// UserActions has NO index beyond its clustered PK (checked live via sys.indexes), so a
			// correlated self-join here — tried first — took 45s+ for a 5-day window (a full scan of a
			// multi-million-row table per Save row). Fixed by matching client-side instead: `generic`
			// already contains every Authorize row in this same date range from the one query below, so
			// buildSaveAuthorizers()/findSaveAuthorizer() do the nearest-preceding-within-60s matching
			// after the fetch, at zero extra SQL cost.
			//
			// Return amount: InvTransactionMaster (holds GrandTotal for every voucher type, keyed by
			// VoucherType) DOES have a usable index on VoucherNumber — but only once matched by that
			// exact number. Save rows don't populate ua.VoucherNumber, so the number is parsed out of
			// the trailing digits of ActionPerformed (e.g. "...SRVAT 7751" -> 7751) and matched via
			// equality (index seek) instead of a date-range scan, which is what made the first version
			...
			const genericSql = `
				...
				OUTER APPLY (
					SELECT TOP 1 u3.UserName AS ShiftUser
					FROM CounterShift cs
					LEFT JOIN Users u3 ON u3.UserID = cs.OpenUserID AND u3.BranchID = ${erpBranchId}
					WHERE ua.ActionName = 'Authorize'
					  AND cs.CounterID = ua.CounterID AND cs.BranchID = ${erpBranchId}
					  AND cs.TransactionDate BETWEEN DATEADD(day, -1, CAST(ua.DateTimeOfAction AS DATE)) AND CAST(ua.DateTimeOfAction AS DATE)
					  AND DATEADD(second, DATEDIFF(second, 0, cs.OpenTime), CAST(cs.TransactionDate AS DATETIME)) <= ua.DateTimeOfAction
					  AND (
					    cs.OpenCloseStatus = 'O'
					    OR DATEADD(second, DATEDIFF(second, 0, cs.ClosingTime), CAST(cs.TransactionDate AS DATETIME)) >= ua.DateTimeOfAction
					    OR DATEADD(second, DATEDIFF(second, 0, cs.ClosingTime), DATEADD(day, 1, CAST(cs.TransactionDate AS DATETIME))) >= ua.DateTimeOfAction
					  )
			...
			// Every Authorize row already comes back from genericSql (it has no ActionName filter) — reuse
			// it here instead of a second query. Matches the nearest Authorize on the same counter that
			// happened AT OR BEFORE a Save, within 60s (mirrors the SQL window that was too slow to run
			// server-side — see the comment above genericSql).
			const authorizeEvents = generic
				.filter((r: any) => r.ActionName === 'Authorize' && r.CounterID != null)
				.map((r: any) => ({ counterId: String(r.CounterID), ms: new Date(r.DateTimeOfAction).getTime(), userName: r.UserName }))
				.filter((e: any) => !Number.isNaN(e.ms));

			function findSaveAuthorizer(counterId: string | null, ms: number): string | null {
				if (counterId == null || Number.isNaN(ms)) return null;
				let best: { ms: number; userName: string } | null = null;
				for (const e of authorizeEvents) {
					if (e.counterId !== counterId) continue;
					const diff = ms - e.ms; // must be >=0: the Authorize precedes the Save, never follows it
			...
			const genericEntries: Entry[] = generic
				.map((r: any) => ({
					...
					counter: r.CounterName,
					counterId: r.CounterID != null ? String(r.CounterID) : null,
					ms: new Date(r.DateTimeOfAction).getTime(),
					kind: deriveKind(r.ActionName, r.ActionPerformed, r.ActionForm),
					isAuthorize: r.ActionName === 'Authorize',
					isSave: r.ActionName === 'Save',
					userName: r.UserName,
					// Who had the till open at the moment of an Authorize event, from the CounterShift
					// join above — falls back to '-' when no shift row covers that timestamp (e.g. gaps
					// in shift logging) rather than misleadingly guessing.
...
<div class="user-action-reports">
	...
	<div class="header">
		<div class="header-left">
			...
			<div>
				<h2 class="header-title">{$t('nav.userActionReports')}</h2>
				<p class="header-subtitle">{$t('userActionReports.subtitle')}</p>
			</div>
		</div>
	</div>
	{/if}
	...
	<div class="filters-panel">
		...
		<div class="filter-field">
			<label for="date-from">{$t('userActionReports.fromLabel')}</label>
			<input id="date-from" type="date" bind:value={dateFrom} />
		</div>
		<div class="filter-field">
			<label for="date-to">{$t('userActionReports.toLabel')}</label>
			<input id="date-to" type="date" bind:value={dateTo} />
		</div>
		<div class="filter-field">
			<label for="cashier-filter">{$t('userActionReports.cashier')}</label>
			<select id="cashier-filter" bind:value={selectedCashier}>
				<option value={null}>{$t('userActionReports.allCashiers')}</option>
		...
		<button class="run-btn" on:click={loadReport} disabled={loading || !selectedBranchId}>
			{loading ? $t('userActionReports.loadingBtn') : $t('userActionReports.runReport')}
		</button>
	</div>
	...
	{#if cappedItemRows || cappedGenericRows}
		<div class="error-banner warn">{$t('userActionReports.resultsCapped', { cap: ROW_CAP })}</div>
	{/if}

	{#if totalItems > 0}
		<div class="summary-cards">
			<div class="summary-card">
				<div class="summary-label">{$t('userActionReports.totalItems')}</div>
				<div class="summary-value">{totalItems}</div>
			</div>
			<div class="summary-card">
				<div class="summary-label">{$t('userActionReports.totalAmount')}</div>
				<div class="summary-value">{formatAmount(totalAmount)}</div>
			</div>
			<div class="summary-card">
				<div class="summary-label">{$t('userActionReports.cancelVoid')}</div>
				<div class="summary-value">{formatAmount(cancelAmount)} / {formatAmount(voidAmount)}</div>
			</div>
			<div class="summary-card">
				<div class="summary-label">{$t('userActionReports.topUser')}</div>
				<div class="summary-value small">{topCashier.name} <span class="dim">({formatAmount(topCashier.amount)})</span></div>
			</div>
			<div class="summary-card">
				<div class="summary-label">{$t('userActionReports.topAuthorizer')}</div>
	...
	{#if summaryRows.length > 0}
		<div class="section-block">
			<div class="section-title-row">
				<h3 class="section-title">{$t('userActionReports.allActionsBreakdown')}</h3>
				{#if selectedKind}
					<button class="clear-filter-btn" on:click={() => (selectedKind = null)}>{$t('userActionReports.clearFilter')}</button>
				{/if}
			</div>
			<div class="type-pills">
				{#each summaryRows as row}
					<button
						class="type-pill"
						class:risk={RISK_KINDS.has(row.Kind)}
						class:active={selectedKind === row.Kind}
						on:click={() => toggleKindFilter(row.Kind)}
						title={NO_ITEM_KINDS.has(row.Kind) ? $t('userActionReports.appBehaviorTooltip') : $t('userActionReports.filterByTypeTooltip')}
					>
						<span class="pill-name">{kindLabel(row.Kind)}</span>
	...
	{#if groupedRows.length > 0}
		<div class="content-split">
				...
				<div class="table-wrapper">
					<table>
						<thead>
							<tr>
								<th></th>
								<th>{$t('userActionReports.colTime')}</th>
								<th>{$t('userActionReports.colCounter')}</th>
								<th>{$t('userActionReports.colUser')}</th>
								<th>{$t('userActionReports.colAuthorizedBy')}</th>
								<th>{$t('userActionReports.colType')}</th>
								<th>{$t('userActionReports.colDetail')}</th>
								<th>{$t('userActionReports.colAmount')}</th>
							</tr>
						</thead>
						<tbody>
			...
			<div class="cashier-column section-block">
				<h3 class="section-title">{$t('userActionReports.userBreakdown')}</h3>
					...
					{#each cashierBreakdown as c}
						<div class="cashier-mini-card" class:active={selectedCashier === c.name} on:click={() => (selectedCashier = selectedCashier === c.name ? null : c.name)}>
							...
							<div class="cashier-mini-stats">
								<span class="cashier-mini-count">{c.bills} {c.bills === 1 ? $t('userActionReports.billWord') : $t('userActionReports.billsWord')}</span>
	...
	{:else if !loading && totalItems === 0 && !errorMessage}
		<div class="empty-state">
			{hasRun
				? $t('userActionReports.noResultsMatchFilters')
				: $t('userActionReports.noDataYet')}
		</div>
	{/if}
</div>
...
<style>
	...
	/* Green is the default — only RISK_KINDS (Void/Cancel/Sales Return, see script) get the .risk
	   variant below, which keeps the original red styling this whole page otherwise uses. */
	.type-pill {
		display: flex;
		align-items: center;
		gap: 0.5rem;
	...
	.type-pill.risk:hover {
		background: rgba(254, 202, 202, 0.9);
	}
	.type-pill.risk .pill-name {
		color: #991b1b;
	}
	.type-pill.risk .pill-count {
		background: #dc2626;
	}
	.type-pill.risk.active {
		background: linear-gradient(135deg, #ef4444, #dc2626);
		border-color: #dc2626;
		box-shadow: 0 4px 12px rgba(220, 38, 38, 0.35);
	}
	.type-pill.risk.active .pill-count {
		color: #b91c1c;
	}
	...
	.cashier-mini-card.active .cashier-mini-count,
