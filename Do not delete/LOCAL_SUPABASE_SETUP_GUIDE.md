...
### ❌ Phase 7: Data Sync System (PENDING)
Build a sync mechanism in the main Aqura app:

1. **Sync Button in StorageManager.svelte** or a new SyncManager component
2. **RPC function on cloud** that exports changed data since last sync
3. **RPC function on local** that imports the data
4. **Sync via Cloudflare tunnel** (`https://erp-branch3.urbanaqura.com`)

```
Cloud Supabase ──► Export JSON ──► Tunnel ──► Local Supabase ──► Import JSON
```

**Sync approach:**
- Each table gets a `updated_at` timestamp column
- Sync tracks `last_sync_timestamp`
- On sync: query all rows where `updated_at > last_sync_timestamp`
- Send to local via API call through tunnel
- Local inserts/updates (upsert)
...
### ❌ Phase 9: Cloudflare Tunnel for Local (PENDING)
1. Install cloudflared on the Ubuntu VM
2. Create tunnel for local Supabase API
3. This allows the cloud Aqura to push data to local
