-- queue_push_notification() is dead code (it reads a non-existent notifications.body column and
-- writes to a non-existent notification_queue table), so calling it from here never delivered
-- anything. Real push goes through the client -> send-push-notification edge function, so this
-- RPC now just returns the notification id for the caller to push. Same signature as before.
