...
export const POST: RequestHandler = async ({ request, fetch, url }) => {
  try {
    ...
    const headlineLayout = /3\s*أيام\s*فقط/.test(input.data.offerName)
      ? 'For this headline use two lines: "عرض خاص" on the first line and "3 أيام فقط" on the second line.'
      : '';
...
Top y=0..535: your scene and sign, with the sign/plaque/banner centered at x=230..865,y=95..435. Render this exact Arabic headline text on it, spelled and worded exactly as given, as bold dimensional 3D typography — extruded/carved lettering with real depth, shading and a highlight, filling the sign attractively and staying legible: ${JSON.stringify(input.data.offerName)}. Arabic layout is strictly RIGHT-TO-LEFT. If a numeral occurs inside the Arabic headline, keep it at its logical reading position; never reorder the surrounding words around it. In the phrase "3 أيام فقط", the visible order across the canvas from RIGHT TO LEFT must be exactly: "3" then "أيام" then "فقط" — the numeral 3 must appear first at the right-hand side of that phrase, never between "أيام" and "فقط" and never after "فقط". ${headlineLayout} Leave x=15..220,y=0..170 as plain, unobstructed background from your scene — no logo, no wordmark, no text, no plaque, no decorative object placed there at all — that exact box is reserved empty for the real logo file the application overlays afterward. Do not add any other text, fake logos, or a second copy of the headline anywhere else in the image.
Middle y=550..1345: this is the product-card zone, but it MUST remain a seamless continuation of the top scene rather than looking like a separate section. Extend the same supermarket environment, campaign colors, lighting, depth, surface, bokeh, and edge motifs naturally downward behind and around the future card grid. There must be NO hard horizontal divider, white/ivory parchment block, isolated panel, abrupt color change, or blank rectangular field between the header and product area. Keep the portions behind the future cards soft, low-contrast, and visually quiet, while allowing restrained scene detail and matching decoration to remain visible in the outer margins and gaps. Absolutely NO products, card outlines, price labels, badges, logos, or writing in this area.
Bottom y=1360..1536: create an attractive premium footer scene styled to match the TOP scene, this offer's context, and the chosen color palette. Fill the otherwise empty footer with a restrained arrangement of realistic, context-related decorative objects, soft highlights, bokeh, ribbons, leaves, particles, or dimensional ornamental details as appropriate to this specific campaign. Keep a clear, quiet central area at x=265..760,y=1370..1525 because the application overlays the complete expiry-date calendar section there. Do NOT draw calendar cards, date plaques, arrows, labels, dates, writing surfaces, or shapes resembling interface panels in that reserved area. Preserve a clear quiet area at x=900..1005,y=1460..1525 for the page number. Do not draw a page number, slogan, logo, product package, price, badge, letter, word, numeral, or pseudo-writing anywhere in the footer.
