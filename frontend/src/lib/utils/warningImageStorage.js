 ...
 */
export function generateWarningImagePath(warningReference, employeeName = 'Unknown', employeeId = null) {
	const now = new Date();
 ...
 */
export async function uploadWarningImage(imageBlob, warningReference, employeeName = 'Unknown', employeeId = null) {
	try {
		// Generate unique path with employee name
		const filePath = generateWarningImagePath(warningReference, employeeName, employeeId);
 ...
 */
export function generateWhatsAppShareUrl(imageUrl, recipientName, message = '') {
 ...
 */
export async function captureAndSaveWarning(element, warningId, warningReference, employeeName = 'Unknown', employeeId = null) {
	try {
		...
		const uploadResult = await uploadWarningImage(imageBlob, warningReference, employeeName, employeeId);
		if (!uploadResult.success) {
			throw new Error(uploadResult.error);
		}
		
		// Step 3: Update database record
		const updateResult = await updateWarningWithImageUrl(warningId, uploadResult.publicUrl);
