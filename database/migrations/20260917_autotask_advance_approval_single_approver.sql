...
-- Fix: Autotask_ingest_receiving_certificate was sending the advance-payment
-- approval request to every approver whose limit covered the amount at once.
-- It must instead route to exactly one approver at a time: the one with the
-- lowest configured limit that still covers the amount (i.e. walking the
-- approval chain from the lowest applicable limit upward). Everything else
-- in this function is unchanged from the live definition.
...
as $$
declare v_row jsonb; v_event public."Autotask_events"; v_rule record; v_users uuid[]; v_user uuid; v_task_id uuid; v_created integer:=0; v_branch text; v_key text; v_amount numeric; v_approvers uuid[]; v_notifications integer:=0; v_vendor_name text; v_received_by text; v_receiving_date text;
begin
  ...
  -- Route to exactly one approver: the lowest configured limit that still
  -- covers the amount. Limit 0 means unlimited, so it only wins when no
  -- smaller, sufficient limit exists. This walks the chain "lowest applicable
  -- limit upward" instead of notifying every qualifying approver at once.
