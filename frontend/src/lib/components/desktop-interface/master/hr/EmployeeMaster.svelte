<script lang="ts">
	...
	let draftDocDays: number | undefined = 65;
	let draftDocType = 'id';
	function openDocFilter() {
		draftDocDays = docDaysLimit ?? 65;
		draftDocType = docFilterType;
		showDocFilterModal = true;
	}
	function applyDocFilter() {
		if (!Number.isInteger(draftDocDays) || Number(draftDocDays) <= 0) return;
		docDaysLimit = Number(draftDocDays);
		docFilterType = draftDocType;
		showDocFilterModal = false;
	}
	let docExcludeStatuses: string[] = ['Resigned'];
	...
	$: docVisibleColumns = Object.fromEntries(Object.entries(columnVisibility).map(([key, visible]) => [key, docDaysLimit !== null && key.startsWith('doc_') ? key === `doc_${docFilterType}` : visible]));
	...
	$: docFilteredData = documentsExpiryData.filter(emp => {
		...
		const matchesStatus = !docExcludeStatuses.includes(emp.employment_status);
		const matchesSponsorship = !docSelectedSponsorship || (emp.sponsorship_status === true) === (docSelectedSponsorship === 'on');
		const selectedDoc = emp.documents?.[docFilterType];
		const matchesRemaining = docDaysLimit === null || (
			selectedDoc?.expiryDate && Number.isFinite(selectedDoc.daysRemaining) &&
			selectedDoc.daysRemaining >= 0 && selectedDoc.daysRemaining < docDaysLimit
		);
...
<!-- ============================================================ -->
	...
	<div class="flex flex-wrap gap-3 p-4 bg-white border-b border-slate-200">
		...
		<div class="relative">
			<button on:click={() => showColumnDropdown = !showColumnDropdown} class="border border-slate-300 rounded-lg px-3 py-2 text-sm bg-white hover:bg-slate-50 flex items-center gap-2">
				🔧 {$t('employeeMaster.docExpiry.columns')} <span class="text-xs text-slate-500">{Object.values(docVisibleColumns).filter(Boolean).length}/{COLUMN_LABELS_EXP.length}</span>
			</button>
	...
	<div class="em-status-toggles">
		<span class="em-status-label">{$t('employeeMaster.allStatuses')}:</span>
		{#each docStatusOptions as s}
			{@const excluded = docExcludeStatuses.includes(s)}
			<label class="em-status-chk" class:excluded style="--sbg:{statusBg(s)};--sfg:{statusColor(s)}">
				<input type="checkbox" checked={!excluded} on:change={() => {
					docExcludeStatuses = excluded ? docExcludeStatuses.filter(status => status !== s) : [...docExcludeStatuses, s];
				}} />
				{getStatusLabel(s)}
			</label>
		{/each}
	...
	<div class="overflow-x-auto">
		<table class="em-table">
			<thead>
				<tr>
					{#if docVisibleColumns.id}<th>{$t('employeeMaster.docExpiry.colId')}</th>{/if}
					{#if docVisibleColumns.name}<th>{$t('employeeMaster.docExpiry.colName')}</th>{/if}
					...
					{#if docVisibleColumns.doc_contract}<th style="text-align:center">{$t('employeeMaster.docExpiry.colContract')}</th>{/if}
					{#if docVisibleColumns.doc_work_permit}<th style="text-align:center">{$t('employeeMaster.docExpiry.colWorkPermit')}</th>{/if}
					{#if docVisibleColumns.doc_insurance}<th style="text-align:center">{$t('employeeMaster.docExpiry.colInsurance')}</th>{/if}
					{#if docVisibleColumns.doc_health_educational}<th style="text-align:center">{$t('employeeMaster.docExpiry.colHealthEdu')}</th>{/if}
				</tr>
			</thead>
			<tbody>
				...
				<tr><td colspan={Math.max(1, Object.values(docVisibleColumns).filter(Boolean).length)} class="text-center py-12 text-slate-400">{$t('employeeMaster.docExpiry.noEmployees')}</td></tr>
				{/if}
			</tbody>
		</table>
	</div>
...
{#if showDocFilterModal}
<div class="em-overlay" role="presentation" on:click|self={() => showDocFilterModal = false}>
	<form class="em-modal" role="dialog" aria-modal="true" aria-labelledby="doc-filter-title" dir={isRTL ? 'rtl' : 'ltr'} on:submit|preventDefault={applyDocFilter}>
		<div class="em-modal-header"><h2 id="doc-filter-title" class="em-modal-title">{$t('employeeMaster.docExpiry.filter')}</h2></div>
		<div class="em-modal-body">
			<label for="doc-filter-days">{$t('employeeMaster.docExpiry.remainingLessThan')}</label>
			<input id="doc-filter-days" type="number" min="1" step="1" required list="doc-filter-presets" bind:value={draftDocDays} class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm" />
			<datalist id="doc-filter-presets"><option value="65" /><option value="30" /><option value="10" /></datalist>
			<label for="doc-filter-type">{$t('employeeMaster.docExpiry.documentType')}</label>
			<select id="doc-filter-type" bind:value={draftDocType} class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm">
				{#each DOCUMENT_TYPES_EXP as doc}
					<option value={doc.type}>{$t(`employeeMaster.docExpiry.documentTypes.${doc.type}`)}</option>
				{/each}
			</select>
		</div>
		<div class="em-modal-footer">
			<button type="button" class="em-btn-cancel" on:click={() => showDocFilterModal = false}>{$t('employeeMaster.docExpiry.cancel')}</button>
			<button type="submit" class="em-btn-save" disabled={!Number.isInteger(draftDocDays) || Number(draftDocDays) <= 0}>{$t('employeeMaster.docExpiry.filter')}</button>
		</div>
	</form>
</div>
{/if}
