...
export async function runGlobalCleanup(): Promise<void> {
  ...
  try {
    const result =
      await PushSubscriptionCleanupService.cleanupAllUserSubscriptions();
...
export async function cleanupInactive(
  ...
  console.log(
    `🧹 Starting cleanup of subscriptions inactive for ${daysInactive} days...`,
  );

  try {
    const result =
      await PushSubscriptionCleanupService.cleanupInactiveSubscriptions(
        daysInactive,
      );
...
export async function getCleanupStats(): Promise<void> {
  ...
  try {
    ...
    if (stats.usersWithSubscriptions > 0) {
      ...
      console.log(
        `   Average active subscriptions per user: ${avgPerUser.toFixed(2)}`,
      );
    }
  } catch (error) {
...
export function initAdminCleanupConsole(): void {
  if (typeof window !== "undefined") {
    ...
    console.log(
      "   - window.cleanupInactive(days) - Clean up inactive subscriptions",
    );
