					...
					try {
						const { error: notifError } = await supabase
							...
							.insert({
								...
								message_en: `Leave request for ${userEmployeeName} (${userEmployeeId}) from ${startDate} to ${endDate} (${dateArray.length} days) requires approval`,
								message_ar: `طلب إجازة للموظف ${userEmployeeNameAr} (${userEmployeeId}) من ${startDate} إلى ${endDate} (${dateArray.length} أيام) يتطلب موافقة`,
								priority: 'high',
