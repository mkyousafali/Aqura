...
// AQ-SEC-005: the Google API key is no longer fetched into the browser — TTS now goes
// through the server proxy at /api/google-ai-proxy, same as Vision/Gemini/Routes.
...
export async function speakWithGoogleTTS(
	...
	try {
		const response = await fetch('/api/google-ai-proxy', {
			...
			body: JSON.stringify({ service: 'tts', body: requestBody })
		});
