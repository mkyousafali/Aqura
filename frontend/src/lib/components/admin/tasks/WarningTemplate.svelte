<script>
	...
	const translations = {
		en: {
			...
			warningSaved: 'Warning saved successfully!',
		...
		ml: {
			...
			edit: 'എഡിറ്റ്',
			print: 'പ്രിന്റ്',
			sendNotification: 'അറിയിപ്പ് അയയ്ക്കുക',
	...
	async function checkWhatsAppContact() {
		try {
			const { data: contactData, error } = await supabase
				...
				.eq('employee_username', data.recipientUsername)
				.single();
	...
	async function saveWarningRecord() {
		...
		try {
			// Get current user information from custom auth store
			...
			const warningRecord = {
				...
				issued_by_username: currentUser.username || 'system',
	...
	async function saveWarningOnly() {
		isSaving = true;
		saveError = null;
		...
		try {
			if (!warningSaved) {
				await saveWarningRecord();
				alert(t.warningSaved || 'Warning saved successfully!');
			} else {
				alert('Warning already saved!');
			}
			// Don't close the modal, allow user to still send notifications or print
		} catch (err) {
			console.error('Error saving warning:', err);
			saveError = 'Failed to save warning: ' + err.message;
		} finally {
			isSaving = false;
		}
	}
		...
		<div class="template-footer">
			<div class="action-buttons">
				...
				<button 
					...
					on:click={sendInternalNotification}
					disabled={isSending}
				>
					{#if isSending}
						<div class="loading-spinner"></div>
